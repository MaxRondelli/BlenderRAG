...
def create_metal_frame_material(name):
    """Materiale cornici metalliche gunmetal scure"""
...
def create_supertall_tower(x_pos, name, height, base_w, top_w):
    """Grattacielo supertall realistico con profilo più angolare"""
    ...
    for floor in range(floors + 1):
        ...
        # Interpolazione larghezza più angolare e geometrica
        curve_factor = 1 - (0.4 * progress * progress)
        width = base_w - (base_w - top_w) * progress * curve_factor
        depth = width * 0.85
        
        # Aggiungi variazione geometrica angolare
        offset_x = 0.2 * math.sin(progress * math.pi * 2) if progress > 0.3 else 0
...
def create_twisted_modern(x_pos, name, height, width):
    """Torre twisted moderna con geometria più angolare"""
    ...
    for floor in range(floors + 1):
        ...
        # Rotazione con steps più definiti per geometria angolare
        angle = max_rotation * progress * progress
        if floor % 5 == 0:
            angle += math.radians(2)
        
        # Dimensione variabile con transizioni più brusche
        scale_factor = 1.0 - 0.3 * (progress ** 1.5)
        ...
        # Vertici ruotati con forma più angolare
        points = [
            (-w/2, -d/2),
            (w/2, -d/2),
            (w/2, d/2),
...
def create_parametric_tower(x_pos, name, height):
    """Torre con geometria parametrica più angolare"""
    ...
    for floor in range(floors + 1):
        ...
        # Forma che si espande e contrae con pattern più geometrico
        base = 12
        variation = 4 * math.sin(progress * math.pi * 4)
        if floor % 8 == 0:
            variation *= 1.3
        ...
        # Aggiungi spigoli più definiti
        chamfer = 0.3 if floor % 10 == 0 else 0
        
        v1 = bm.verts.new((x_pos - width/2 + chamfer, -depth/2, z))
        v2 = bm.verts.new((x_pos + width/2 - chamfer, -depth/2, z))
        v3 = bm.verts.new((x_pos + width/2 - chamfer, depth/2, z))
        v4 = bm.verts.new((x_pos - width/2 + chamfer, depth/2, z))
...
glass2 = create_modern_glass_material("BronzeGlass", (0.75, 0.45, 0.25, 1))
...
tower3 = create_parametric_tower(35, "ParametricRed", 95)
glass3 = create_modern_glass_material("RedGlass", (0.65, 0.15, 0.15, 1), is_dark=True)
...
print("✓ 3 grattacieli fotorealistici con colori caldi:")
print("  - Supertall angolare (ambra)")
print("  - Twisted geometrico (bronzo)")
print("  - Parametrico spigoloso (rosso scuro)")
print("  - Cornici gunmetal scure industriali")
