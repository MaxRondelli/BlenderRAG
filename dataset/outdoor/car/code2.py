...
# Create simple main car body - just one elongated box (lower and more aerodynamic)
bpy.ops.mesh.primitive_cube_add(size=2, location=(0, 0, 0.65))
car_body = bpy.context.active_object
car_body.name = "CarBody"
car_body.scale = (2.5, 1.0, 0.25)

# Create simple cabin - smaller box on top (significantly smaller and more angular)
bpy.ops.mesh.primitive_cube_add(size=2, location=(-0.3, 0, 1.25))
car_cabin = bpy.context.active_object
car_cabin.name = "CarCabin"
car_cabin.scale = (1.0, 0.8, 0.35)
...
# Create and assign materials (deep metallic blue for sports car)
body_mat = create_material("BodyMat", (0.1, 0.2, 0.85, 1.0), metallic=0.9, roughness=0.1)  # Deep metallic blue
...
print("Sports car created successfully!")
