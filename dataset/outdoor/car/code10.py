...
# Create sleek sports car body - longer and lower
bpy.ops.mesh.primitive_cube_add(size=2, location=(0, 0, 0.65))
car_body = bpy.context.active_object
car_body.name = "CarBody"
car_body.scale = (2.5, 1.2, 0.25)

# Create angular cabin - lower and longer
bpy.ops.mesh.primitive_cube_add(size=2, location=(-0.3, 0, 1.1))
car_cabin = bpy.context.active_object
car_cabin.name = "CarCabin"
car_cabin.scale = (1.5, 0.88, 0.35)
...
headlights = [
    create_light(2.6, 0.75, 0.6, "HeadlightRight"),
    create_light(2.6, -0.75, 0.6, "HeadlightLeft")
]

# Create simple taillights
taillights = [
    create_light(-2.6, 0.75, 0.65, "TaillightRight"),
    create_light(-2.6, -0.75, 0.65, "TaillightLeft")
]
...
body_mat = create_material("BodyMat", (0.1, 0.2, 0.7, 1.0), metallic=0.9, roughness=0.02)  # Deep metallic blue with high gloss
...
bpy.context.active_object.name = "SleekSportsCar"

print("Sleek sports car created successfully!")
