...
# Create simple main car body - just one elongated box (sports car: longer and lower)
bpy.ops.mesh.primitive_cube_add(size=2, location=(0, 0, 0.65))
car_body = bpy.context.active_object
car_body.name = "CarBody"
car_body.scale = (2.5, 1.2, 0.25)  # Sports car: longer (y=1.2) and lower (z=0.25)

# Create simple cabin - smaller box on top (adjusted for lower profile)
bpy.ops.mesh.primitive_cube_add(size=2, location=(-0.3, 0, 1.15))
car_cabin = bpy.context.active_object
car_cabin.name = "CarCabin"
car_cabin.scale = (1.3, 1.05, 0.35)

# Function to create realistic wheel (sports car: larger wheels)
def create_wheel(x, y, name):
    z = 0.5  # Wheel height from ground (slightly higher for larger wheels)
    
    # Main tire (torus for realistic profile) - larger
    bpy.ops.mesh.primitive_torus_add(
        major_radius=0.45,  # Larger radius
        minor_radius=0.17,  # Larger minor radius
        location=(x, y, z),
...
# Create wheels - perfectly symmetric positioning (adjusted for longer body)
front_x = 1.7
rear_x = -1.7
wheel_y = 1.3  # Distance from center (adjusted for wider body)
...
# Create simple headlights (adjusted for longer body)
headlights = [
    create_light(2.7, 0.75, 0.6, "HeadlightRight"),
    create_light(2.7, -0.75, 0.6, "HeadlightLeft")
]

# Create simple taillights (adjusted for longer body)
taillights = [
    create_light(-2.7, 0.75, 0.65, "TaillightRight"),
    create_light(-2.7, -0.75, 0.65, "TaillightLeft")
]
...
body_mat = create_material("BodyMat", (0.1, 0.2, 0.8, 1.0), metallic=0.9, roughness=0.3)  # Deep blue metallic with higher roughness
tire_mat = create_material("TireMat", (0.03, 0.03, 0.03, 1.0), metallic=0.0, roughness=0.9)  # Rubber
rim_mat = create_material("RimMat", (0.05, 0.05, 0.05, 1.0), metallic=0.95, roughness=0.05)  # Black rims instead of chrome
...
print("Sports car variation created successfully!")
