...
# Reduced bending for more upright appearance
bpy.ops.mesh.select_all(action='DESELECT')
bpy.ops.object.mode_set(mode='OBJECT')

# Move vertices for reduced curvature
for i, vert in enumerate(grass_blade.data.vertices):
    if vert.co.z > 0.3:
        vert.co.x += 0.02 * vert.co.z
        vert.co.z *= 0.95

bpy.ops.object.mode_set(mode='EDIT')
bpy.ops.object.mode_set(mode='OBJECT')

# Autumn grass material with golden-brown gradient
mat_grass = bpy.data.materials.new("AutumnGrassMat")
...
# Autumn colors: golden-brown to amber
ramp.color_ramp.elements[0].color = (0.25, 0.15, 0.08, 1)
ramp.color_ramp.elements[1].color = (0.8, 0.6, 0.25, 1)
...
pset.count = 38000
pset.hair_length = 1.4
pset.use_rotations = True
pset.rotation_mode = 'NOR'
pset.phase_factor_random = 1.5
pset.particle_size = 1.3
pset.size_random = 0.5
pset.length_random = 0.4

# Children
pset.child_type = 'SIMPLE'
pset.rendered_child_count = 8
pset.child_radius = 0.3
pset.child_length = 0.9
pset.child_size_random = 0.4

# Increased clumping for distinct patches
pset.clump_factor = 0.6
pset.roughness_1 = 0.4
...
if bg:
    bg.inputs['Color'].default_value = (0.6, 0.5, 0.4, 1)
...
print("✓ Autumn grass with golden-brown colors and increased clumping!")
