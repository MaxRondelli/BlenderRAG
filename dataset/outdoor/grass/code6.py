...
# === REALISTIC AUTUMN GRASS BLADE (BENT PLANE) ===
...
# Bend the tip more pronounced for dried grass
bpy.ops.mesh.select_all(action='DESELECT')
bpy.ops.object.mode_set(mode='OBJECT')

# Move vertices for stronger curvature
for i, vert in enumerate(grass_blade.data.vertices):
    if vert.co.z > 0.2:
        vert.co.x += 0.12 * vert.co.z
        vert.co.z *= 0.85
    if vert.co.z > 0.4:
        vert.co.x += 0.08 * vert.co.z
        vert.co.z *= 0.8
...
# Autumn colors: burnt orange to golden yellow
ramp.color_ramp.elements[0].color = (0.4, 0.15, 0.05, 1)
ramp.color_ramp.elements[1].color = (0.8, 0.6, 0.2, 1)

# Add middle element for reddish-brown
ramp.color_ramp.elements.new(0.3)
ramp.color_ramp.elements[1].color = (0.5, 0.2, 0.1, 1)
...
pset.child_size_random = 0.6

pset.clump_factor = 0.3
pset.roughness_1 = 0.8
...
if bg:
    bg.inputs['Color'].default_value = (0.7, 0.6, 0.5, 1)
...
print("✓ Autumn grass with warm colors and enhanced curvature!")
