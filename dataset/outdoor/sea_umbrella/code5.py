...
def crea_materiale_tessuto():
    ...
    bsdf = nodes.new('ShaderNodeBsdfPrincipled')
    bsdf.inputs['Base Color'].default_value = (0.15, 0.35, 0.75, 1)
...
# Tavolo
bpy.ops.mesh.primitive_cylinder_add(vertices=64, radius=0.6, depth=0.08, location=(0, 0, 0.3))
...
bpy.ops.mesh.primitive_cylinder_add(vertices=32, radius=0.025, depth=2.6, location=(centro_x, centro_y, 1.3))
...
# Hub centrale
altezza_hub = 2.6
bpy.ops.mesh.primitive_uv_sphere_add(radius=0.05, location=(centro_x, centro_y, altezza_hub))
...
for ring_idx in range(1, num_anelli + 1):
    ...
    z_corrente = altezza_hub - (altezza_hub - altezza_bordo) * t - 0.08 * t * (1 - t) * 4
...
# Camera
bpy.ops.object.camera_add(location=(4.5, -4.2, 2.5))
...
print("Telo continuo senza buchi!")