...
def crea_materiale_legno():
    ...
    color_ramp.color_ramp.elements[1].color = (0.28, 0.18, 0.10, 1)
...
def crea_materiale_tessuto():
    ...
    color_ramp = nodes.new('ShaderNodeValToRGB')
    color_ramp.color_ramp.elements[0].color = (0.95, 0.95, 0.95, 1)
    color_ramp.color_ramp.elements[1].color = (0.15, 0.45, 0.85, 1)
...
# Tavolo
bpy.ops.mesh.primitive_cylinder_add(vertices=64, radius=0.8, depth=0.1, location=(0, 0, 0.35))
...
bpy.ops.mesh.primitive_cylinder_add(vertices=32, radius=0.03, depth=3.0, location=(centro_x, centro_y, 1.5))
...
# Hub centrale
altezza_hub = 3.0
bpy.ops.mesh.primitive_uv_sphere_add(radius=0.06, location=(centro_x, centro_y, altezza_hub))
...
for ring_idx in range(1, num_anelli + 1):
    ...
    # Altezza interpolata con curvatura
    z_corrente = altezza_hub - (altezza_hub - altezza_bordo) * t - 0.1 * t * (1 - t) * 4
...
# Camera
bpy.ops.object.camera_add(location=(6.0, -5.5, 3.2))
...
sole.data.energy = 6.0
sole.data.angle = math.radians(0.53)
sole.rotation_euler = (math.radians(40), math.radians(25), math.radians(130))
