...
mat_base = bpy.data.materials.new(name="BaseChrome")
...
bsdf_palo.inputs['Base Color'].default_value = (0.85, 0.88, 0.92, 1)
bsdf_palo.inputs['Metallic'].default_value = 1.0
bsdf_palo.inputs['Roughness'].default_value = 0.08
palo.data.materials.append(mat_palo)

# CORPO PRINCIPALE - Rectangular/Modern Shape
bpy.ops.mesh.primitive_cube_add(size=1.0, location=(0, 0, 2.9))
corpo = bpy.context.active_object
corpo.scale = (0.22, 0.16, 0.85)

# Add subdivision surface for smoother edges
subsurf_modifier = corpo.modifiers.new(name="Subsurf", type="SUBSURF")
subsurf_modifier.levels = 2

mat_corpo = bpy.data.materials.new(name="CorpoSilver")
mat_corpo.use_nodes = True
bsdf_corpo = mat_corpo.node_tree.nodes["Principled BSDF"]
bsdf_corpo.inputs['Base Color'].default_value = (0.75, 0.8, 0.85, 1)
...
# LUCI CON VISIERE - Modern LED Colors
luci_data = [
    ("Rosso", (1.0, 0.1, 0.05), 3.15),
    ("Giallo", (1.0, 0.8, 0.0), 2.9),
    ("Verde", (0.0, 1.0, 0.2), 2.65)
]

for nome, colore, z_pos in luci_data:
    # VISIERA sopra la luce - Chrome finish
    ...
    mat_anello = bpy.data.materials.new(name=f"Anello_{nome}")
    mat_anello.use_nodes = True
    bsdf_anello = mat_anello.node_tree.nodes["Principled BSDF"]
    bsdf_anello.inputs['Base Color'].default_value = (0.9, 0.92, 0.95, 1)
    bsdf_anello.inputs['Metallic'].default_value = 1.0
    bsdf_anello.inputs['Roughness'].default_value = 0.05
...
print("Modern chrome LED stoplight created!")
