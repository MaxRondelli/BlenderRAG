import bpy
import bmesh
import math

# Clear scene
bpy.ops.object.select_all(action='SELECT')
bpy.ops.object.delete()

# BASE
bpy.ops.mesh.primitive_cube_add(size=2, location=(0, 0, 0.1))
base = bpy.context.active_object
base.scale = (0.5, 0.3, 0.05)

# Add subdivision and smooth base
modifier = base.modifiers.new(name="Subsurf", type="SUBSURF")
modifier.levels = 2
bpy.context.view_layer.objects.active = base
bpy.ops.object.shade_smooth()

mat_base = bpy.data.materials.new(name="BaseBlack")
mat_base.use_nodes = True
bsdf = mat_base.node_tree.nodes["Principled BSDF"]
bsdf.inputs['Base Color'].default_value = (0.05, 0.05, 0.05, 1)
bsdf.inputs['Roughness'].default_value = 0.4
base.data.materials.append(mat_base)

# POLE
bpy.ops.mesh.primitive_cylinder_add(radius=0.04, depth=2.5, location=(0, 0, 1.35))
palo = bpy.context.active_object

# Smooth pole
modifier_palo = palo.modifiers.new(name="Subsurf", type="SUBSURF")
modifier_palo.levels = 2
bpy.context.view_layer.objects.active = palo
bpy.ops.object.shade_smooth()

mat_palo = bpy.data.materials.new(name="PaloChrome")
mat_palo.use_nodes = True
bsdf_palo = mat_palo.node_tree.nodes["Principled BSDF"]
bsdf_palo.inputs['Base Color'].default_value = (0.8, 0.8, 0.85, 1)
bsdf_palo.inputs['Metallic'].default_value = 0.9
bsdf_palo.inputs['Roughness'].default_value = 0.1
palo.data.materials.append(mat_palo)

# MAIN BODY - Rectangular with rounded edges
bpy.ops.mesh.primitive_cube_add(size=1., location=(0, 0, 2.9))
corpo = bpy.context.active_object
corpo.scale = (0.18, 0.14, 0.8)

# Add bevel modifier for rounded edges
bevel_mod = corpo.modifiers.new(name="Bevel", type="BEVEL")
bevel_mod.width = 0.02
bevel_mod.segments = 3

# Add subdivision for smoothness
subsurf_mod = corpo.modifiers.new(name="Subsurf", type="SUBSURF")
subsurf_mod.levels = 2

bpy.context.view_layer.objects.active = corpo
bpy.ops.object.shade_smooth()

# Chrome material for main body
mat_corpo = bpy.data.materials.new(name="CorpoChrome")
mat_corpo.use_nodes = True
nodes = mat_corpo.node_tree.nodes
links = mat_corpo.node_tree.links

bsdf_corpo = nodes["Principled BSDF"]
bsdf_corpo.inputs['Base Color'].default_value = (0.7, 0.7, 0.75, 1)
bsdf_corpo.inputs['Roughness'].default_value = 0.05
bsdf_corpo.inputs['Metallic'].default_value = 0.95

# Add noise for brushed metal effect
noise = nodes.new('ShaderNodeTexNoise')
noise.inputs['Scale'].default_value = 50.0
noise.inputs['Detail'].default_value = 16.0
noise.inputs['Roughness'].default_value = 0.8

colorramp = nodes.new('ShaderNodeValToRGB')
colorramp.color_ramp.elements[0].color = (0.02, 0.02, 0.02, 1)
colorramp.color_ramp.elements[1].color = (0.08, 0.08, 0.08, 1)

links.new(noise.outputs['Fac'], colorramp.inputs['Fac'])
links.new(colorramp.outputs['Color'], bsdf_corpo.inputs['Roughness'])

corpo.data.materials.append(mat_corpo)

# LIGHTS WITH VISORS
luci_data = [
    ("Rosso", (1, 0, 0), 3.25),
    ("Giallo", (1, 0.6, 0), 2.9),
    ("Verde", (0, 1, 0), 2.55)
]

for nome, colore, z_pos in luci_data:
    # VISOR above light - rounded
    bpy.ops.mesh.primitive_cylinder_add(radius=0.11, depth=0.08, location=(0, 0.08, z_pos + 0.08))
    visiera = bpy.context.active_object
    visiera.rotation_euler = (math.radians(90), 0, 0)
    visiera.scale = (1, 0.6, 1)
    
    # Round the visor edges
    bevel_visor = visiera.modifiers.new(name="Bevel", type="BEVEL")
    bevel_visor.width = 0.01
    bevel_visor.segments = 2
    
    subsurf_visor = visiera.modifiers.new(name="Subsurf", type="SUBSURF")
    subsurf_visor.levels = 1
    
    bpy.context.view_layer.objects.active = visiera
    bpy.ops.object.shade_smooth()
    
    visiera.data.materials.append(mat_corpo)
    
    # Light sphere - smoother
    bpy.ops.mesh.primitive_uv_sphere_add(radius=0.075, location=(0, 0.02, z_pos))
    luce = bpy.context.active_object
    
    # Smooth the light
    subsurf_luce = luce.modifiers.new(name="Subsurf", type="SUBSURF")
    subsurf_luce.levels = 2
    
    bpy.context.view_layer.objects.active = luce
    bpy.ops.object.shade_smooth()
    
    # Emissive glass material
    mat_luce = bpy.data.materials.new(name=f"Mat_{nome}")
    mat_luce.use_nodes = True
    nodes_luce = mat_luce.node_tree.nodes
    links_luce = mat_luce.node_tree.links
    nodes_luce.clear()
    
    output = nodes_luce.new('ShaderNodeOutputMaterial')
    mix = nodes_luce.new('ShaderNodeMixShader')
    emission = nodes_luce.new('ShaderNodeEmission')
    glass = nodes_luce.new('ShaderNodeBsdfGlass')
    
    emission.inputs['Color'].default_value = (*colore, 1)
    emission.inputs['Strength'].default_value = 8.0
    glass.inputs['Color'].default_value = (*colore, 1)
    glass.inputs['IOR'].default_value = 1.45
    glass.inputs['Roughness'].default_value = 0.02
    
    links_luce.new(emission.outputs[0], mix.inputs[1])
    links_luce.new(glass.outputs[0], mix.inputs[2])
    links_luce.new(mix.outputs[0], output.inputs[0])
    mix.inputs['Fac'].default_value = 0.8
    
    luce.data.materials.append(mat_luce)
    
    # Black ring - rounded
    bpy.ops.mesh.primitive_torus_add(major_radius=0.095, minor_radius=0.012, location=(0, 0.02, z_pos))
    anello = bpy.context.active_object
    anello.rotation_euler = (math.radians(90), 0, 0)
    
    # Smooth the ring
    subsurf_anello = anello.modifiers.new(name="Subsurf", type="SUBSURF")
    subsurf_anello.levels = 2
    
    bpy.context.view_layer.objects.active = anello
    bpy.ops.object.shade_smooth()
    
    anello.data.materials.append(mat_corpo)

# CAMERA
bpy.ops.object.camera_add(location=(1.5, -1.8, 2.5))
camera = bpy.context.active_object
camera.rotation_euler = (math.radians(70), 0, math.radians(40))
bpy.context.scene.camera = camera

# LIGHTS
bpy.ops.object.light_add(type='SUN', location=(5, -5, 10))
sun = bpy.context.active_object
sun.data.energy = 3.0

bpy.ops.object.light_add(type='AREA', location=(-2, -2, 3))
area = bpy.context.active_object
area.data.energy = 150

# Add rim light for chrome effect
bpy.ops.object.light_add(type='AREA', location=(2, 1, 4))
rim_light = bpy.context.active_object
rim_light.data.energy = 80
rim_light.data.color = (0.9, 0.95, 1.0)

# WORLD HDRI
world = bpy.context.scene.world
world.use_nodes = True
bg = world.node_tree.nodes['Background']
bg.inputs['Strength'].default_value = 0.5



# RENDER SETTINGS
scene = bpy.context.scene
scene.render.engine = 'CYCLES'
scene.cycles.samples = 512
scene.render.resolution_x = 1920
scene.render.resolution_y = 1080
scene.view_settings.view_transform = 'Filmic'

print("Modern chrome stoplight created!")


