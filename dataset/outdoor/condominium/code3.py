...
# ============================================================
# VARIATION: SLEEK MODERN MATERIALS
# ============================================================
...
def mat_concrete():
    ...
    safe_set(bsdf.inputs, 'Roughness', 0.75)
    safe_set(bsdf.inputs, 'Metallic', 0.0)
    
    links.new(bsdf.outputs['BSDF'], out.inputs['Surface'])
    return m

# DARKER CHARCOAL CONCRETE
def mat_concrete_dark():
    m = bpy.data.materials.new("ConcreteCharcoalDark")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
    ...
    ramp = nodes.new('ShaderNodeValToRGB')
    ramp.color_ramp.elements[0].color = (0.15, 0.15, 0.17, 1)
...
# BRIGHT TEAL GLASS
def mat_glass():
    m = bpy.data.materials.new("TealGlass")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
    ...
    safe_set(bsdf.inputs, 'Base Color', (0.15, 0.85, 0.75, 1))
...
# BLACK POWDER-COATED METAL RAILINGS
def mat_metal_railing():
    m = bpy.data.materials.new("BlackMetalRailing")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
    ...
    mix.data_type = 'RGBA'
    mix.inputs[6].default_value = (0.08, 0.08, 0.10, 1)
    mix.inputs[7].default_value = (0.12, 0.12, 0.14, 1)
...
# MODERN WOOD
def mat_wood():
    m = bpy.data.materials.new("ModernWood")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
    ...
    ramp = nodes.new('ShaderNodeValToRGB')
    ramp.color_ramp.elements[0].color = (0.30, 0.20, 0.12, 1)
    ramp.color_ramp.elements[1].color = (0.50, 0.32, 0.18, 1)
...
for i, (px, py) in enumerate(pilaster_locs):
    ...
    pil.data.materials.append(M_CONC_DARK)
...
for floor in range(P["floors"]):
    ...
    left_wall = box(f"LeftWall_F{floor}",
                   ...
                   col_facade)
    left_wall.data.materials.append(M_CONC)
    
    right_wall = box(f"RightWall_F{floor}",
                    (0.25, P["depth"] - 2*P["pilaster_d"], wall_h),
                    (P["width"]/2 - 0.125, 0, z_base + wall_h/2 + P["slab_h"]),
                    col_facade)
    right_wall.data.materials.append(M_CONC)
...
for floor in range(P["floors"]):
    ...
    seg1 = box(f"FrontWall_F{floor}_Seg1",
              ...
              col_facade)
    seg1.data.materials.append(M_CONC)
    ...
    seg2 = box(f"FrontWall_F{floor}_Seg2",
              ...
              col_facade)
    seg2.data.materials.append(M_CONC)
    ...
    seg3 = box(f"FrontWall_F{floor}_Seg3",
              ...
              col_facade)
    seg3.data.materials.append(M_CONC)
    ...
    seg4 = box(f"FrontWall_F{floor}_Seg4",
              ...
              col_facade)
    seg4.data.materials.append(M_CONC)
    ...
    top_strip = box(f"FrontWall_F{floor}_Top",
                   ...
                   col_facade)
    top_strip.data.materials.append(M_CONC)
    ...
    bot_strip = box(f"FrontWall_F{floor}_Bot",
                   ...
                   col_facade)
    bot_strip.data.materials.append(M_CONC)
    ...
    for i, x in enumerate(front_window_x):
        frame = box(f"WinFrame_F{floor}_F{i}",
                   ...
                   col_facade)
        frame.data.materials.append(M_CONC_DARK)
...
for floor in range(P["floors"]):
    ...
    for j in range(3):
        ...
        seg = box(f"BackWall_F{floor}_Seg{j+2}",
                 ...
                 col_facade)
        seg.data.materials.append(M_CONC)
    ...
    seg5 = box(f"BackWall_F{floor}_Seg5",
              ...
              col_facade)
    seg5.data.materials.append(M_CONC)
...
# ============================================================
# VARIATION: MODERN INDUSTRIAL BALCONIES
# ============================================================
...
for floor in range(P["floors"]):
    ...
    for side, x_sign in [("L", -1), ("R", 1)]:
        ...
        handrail_left = box(f"Handrail_{side}_F{floor}_Left",
                      (0.06, P["balcony_depth"] - 0.2, 0.06),
                      (x_pos - P["balcony_w"]/2 + 0.05, y_pos, 
                       z_base + 0.2 + P["railing_h"]),
                      col_balconies)
        handrail_left.data.materials.append(M_RAILING)
        
        for h_offset in [0.3, 0.6]:
            rail = box(f"Rail_{side}_F{floor}_Left_h{int(h_offset*10)}",
                      (0.04, P["balcony_depth"] - 0.2, 0.025),
                      (x_pos - P["balcony_w"]/2 + 0.05, y_pos,
                       z_base + 0.2 + h_offset),
                      col_balconies)
            rail.data.materials.append(M_RAILING)
        ...
        handrail_right = box(f"Handrail_{side}_F{floor}_Right",
                      ...
                      (x_pos + P["balcony_w"]/2 - 0.05, y_pos, 
                       z_base + 0.2 + P["railing_h"]),
                      col_balconies)
        handrail_right.data.materials.append(M_RAILING)
...
print("✅ Sleek modern condominium with charcoal facades and teal glass - F12 for render")
