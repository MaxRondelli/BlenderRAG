...
def rounded_box(name, size, loc, col=None, corner_radius=0.2):
    sx, sy, sz = size
    bm = bmesh.new()
    bmesh.ops.create_cube(bm, size=1.0)
    bmesh.ops.scale(bm, vec=(sx, sy, sz), verts=bm.verts)
    bmesh.ops.bevel(bm, geom=bm.edges[:], offset=min(corner_radius, min(sx, sy, sz) * 0.1), segments=3)
    mesh = bpy.data.meshes.new(name)
    bm.to_mesh(mesh)
    bm.free()
    obj = bpy.data.objects.new(name, mesh)
...
# CEMENTO SCURO CHARCOAL
def mat_charcoal_concrete():
    m = bpy.data.materials.new("CharcoalConcrete")
    m.use_nodes = True
    ...
    ramp.color_ramp.elements[0].color = (0.15, 0.15, 0.16, 1)
    ramp.color_ramp.elements[1].color = (0.25, 0.25, 0.27, 1)
...
# CEMENTO CHARCOAL PIÙ SCURO
def mat_dark_charcoal():
    m = bpy.data.materials.new("DarkCharcoal")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
    ...
    ramp = nodes.new('ShaderNodeValToRGB')
    ramp.color_ramp.elements[0].color = (0.08, 0.08, 0.09, 1)
    ramp.color_ramp.elements[1].color = (0.18, 0.18, 0.20, 1)
    ...
    safe_set(bsdf.inputs, 'Roughness', 0.85)
    safe_set(bsdf.inputs, 'Metallic', 0.0)
...
# VETRO INDUSTRIALE
def mat_industrial_glass():
    m = bpy.data.materials.new("IndustrialGlass")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
    ...
    safe_set(bsdf.inputs, 'Base Color', (0.88, 0.92, 0.95, 1))
    safe_set(bsdf.inputs, 'Roughness', 0.02)
    safe_set(bsdf.inputs, 'IOR', 1.52)
    safe_set(bsdf.inputs, 'Alpha', 0.15)
...
# RAME BRILLANTE
def mat_copper_metal():
    m = bpy.data.materials.new("CopperMetal")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
    ...
    mix.data_type = 'RGBA'
    mix.inputs[6].default_value = (0.95, 0.45, 0.15, 1)
    mix.inputs[7].default_value = (0.85, 0.35, 0.08, 1)
...
def mat_modern_glass():
    ...
    safe_set(bsdf.inputs, 'Base Color', (0.85, 0.90, 0.92, 1))
...
def mat_dark_steel():
    ...
    safe_set(bsdf.inputs, 'Base Color', (0.12, 0.12, 0.14, 1))
...
M_CHARCOAL = mat_charcoal_concrete()
M_DARK_CHARCOAL = mat_dark_charcoal()
M_GLASS_IND = mat_industrial_glass()
M_COPPER = mat_copper_metal()
M_MODERN_GLASS = mat_modern_glass()
...
foundation.data.materials.append(M_DARK_CHARCOAL)
...
for i, (px, py) in enumerate(pilaster_locs):
    ...
    pil.data.materials.append(M_DARK_CHARCOAL)
...
for floor in range(P["floors"] + 1):
    ...
    slab = box(f"Slab_{floor}", 
              ...
              (0, 0, z), col_struct)
    slab.data.materials.append(M_CHARCOAL)
...
for floor in range(P["floors"]):
    ...
    left_wall = box(f"LeftWall_F{floor}",
                   ...
                   col_facade)
    left_wall.data.materials.append(M_CHARCOAL)
    
    right_wall = box(f"RightWall_F{floor}",
                    (0.25, P["depth"] - 2*P["pilaster_d"], wall_h),
                    (P["width"]/2 - 0.125, 0, z_base + wall_h/2 + P["slab_h"]),
                    col_facade)
    right_wall.data.materials.append(M_CHARCOAL)
...
for floor in range(P["floors"]):
    ...
    seg1 = box(f"FrontWall_F{floor}_Seg1",
              ...
              col_facade)
    seg1.data.materials.append(M_CHARCOAL)
    ...
    seg2 = box(f"FrontWall_F{floor}_Seg2",
              ...
              col_facade)
    seg2.data.materials.append(M_CHARCOAL)
    ...
    seg3 = box(f"FrontWall_F{floor}_Seg3",
              ...
              col_facade)
    seg3.data.materials.append(M_CHARCOAL)
    ...
    seg4 = box(f"FrontWall_F{floor}_Seg4",
              ...
              col_facade)
    seg4.data.materials.append(M_CHARCOAL)
    ...
    top_strip = box(f"FrontWall_F{floor}_Top",
                   ...
                   col_facade)
    top_strip.data.materials.append(M_CHARCOAL)
    ...
    bot_strip = box(f"FrontWall_F{floor}_Bot",
                   ...
                   col_facade)
    bot_strip.data.materials.append(M_CHARCOAL)
    ...
    for i, x in enumerate(front_window_x):
        frame = box(f"WinFrame_F{floor}_F{i}",
                   ...
                   col_facade)
        frame.data.materials.append(M_COPPER)
        
        glass = box(f"Window_F{floor}_F{i}",
                   (P["window_w"], 0.02, P["window_h"]),
                   (x, front_y, z_window),
                   col_facade)
        glass.data.materials.append(M_GLASS_IND)
...
for floor in range(P["floors"]):
    ...
    for j in range(3):
        ...
        seg = box(f"BackWall_F{floor}_Seg{j+2}",
                 ...
                 col_facade)
        seg.data.materials.append(M_CHARCOAL)
    ...
    seg5 = box(f"BackWall_F{floor}_Seg5",
              ...
              col_facade)
    seg5.data.materials.append(M_CHARCOAL)
...
# ============================================================
# BALCONI CON ANGOLI ARROTONDATI
# ============================================================
...
for floor in range(P["floors"]):
    ...
    for side, x_sign in [("L", -1), ("R", 1)]:
        ...
        balcony = rounded_box(f"Balcony_{side}_F{floor}",
                     (P["balcony_w"], P["balcony_depth"], 0.2),
                     (x_pos, y_pos, z_base + 0.1),
                     col_balconies, 0.15)
        balcony.data.materials.append(M_CHARCOAL)
        
        under = rounded_box(f"BalconyUnder_{side}_F{floor}",
                   (P["balcony_w"] - 0.2, P["balcony_depth"] - 0.2, 0.15),
                   (x_pos, y_pos, z_base - 0.075),
                   col_balconies, 0.1)
        under.data.materials.append(M_CHARCOAL)
        ...
        for i in range(num_posts_front):
            ...
            post = cylinder(f"RailPost_{side}F{floor}_Front{i}",
                          ...
                          col_balconies, v=8)
            post.data.materials.append(M_COPPER)
        ...
        handrail_front = box(f"Handrail_{side}_F{floor}_Front",
                      ...
                      col_balconies)
        handrail_front.data.materials.append(M_COPPER)
        ...
        for h_offset in [0.3, 0.6]:
            rail = box(f"Rail_{side}_F{floor}_Front_h{int(h_offset*10)}",
                      ...
                      (x_pos, y_pos + P["balcony_depth"]/2 - 0.05,
                       z_base + 0.2 + h_offset),
                      col_balconies)
            rail.data.materials.append(M_COPPER)
        ...
        handrail_left = box(f"Handrail_{side}_F{floor}_Left",
                      ...
                      (x_pos - P["balcony_w"]/2 + 0.05, y_pos, 
                       z_base + 0.2 + P["railing_h"]),
                      col_balconies)
        handrail_left.data.materials.append(M_COPPER)
        ...
        handrail_right = box(f"Handrail_{side}_F{floor}_Right",
                      ...
                      (x_pos + P["balcony_w"]/2 - 0.05, y_pos, 
                       z_base + 0.2 + P["railing_h"]),
                      col_balconies)
        handrail_right.data.materials.append(M_COPPER)
...
ph_base.data.materials.append(M_CHARCOAL)

ph_back = box("PH_Back", (14.0, 0.25, 2.8), (0, -5.5, ph_z + 1.4), col_pent)
ph_back.data.materials.append(M_CHARCOAL)

ph_left = box("PH_Left", (0.25, 11.0, 2.8), (-7.0, 0, ph_z + 1.4), col_pent)
ph_left.data.materials.append(M_CHARCOAL)

ph_right = box("PH_Right", (0.25, 11.0, 2.8), (7.0, 0, ph_z + 1.4), col_pent)
ph_right.data.materials.append(M_CHARCOAL)

glass_panel_1 = box("PH_Glass_Panel_1",
                   (4.5, 0.03, 2.4),
                   (-4.75, 5.2, ph_z + 1.6),
                   col_pent)
glass_panel_1.data.materials.append(M_MODERN_GLASS)

frame_1 = box("PH_Frame_1",
             (4.7, 0.08, 2.6),
             (-4.75, 5.15, ph_z + 1.6),
             col_pent)
frame_1.data.materials.append(M_DARK_STEEL)

glass_panel_2 = box("PH_Glass_Panel_2",
                   (4.5, 0.03, 2.4),
                   (-0.5, 5.3, ph_z + 1.6),
                   col_pent)
glass_panel_2.data.materials.append(M_MODERN_GLASS)

frame_2 = box("PH_Frame_2",
             (4.7, 0.08, 2.6),
             (-0.5, 5.25, ph_z + 1.6),
             col_pent)
frame_2.data.materials.append(M_DARK_STEEL)

glass_panel_3 = box("PH_Glass_Panel_3",
                   (4.5, 0.03, 2.4),
                   (4.25, 5.15, ph_z + 1.6),
                   col_pent)
glass_panel_3.data.materials.append(M_MODERN_GLASS)

frame_3 = box("PH_Frame_3",
             (4.7, 0.08, 2.6),
             (4.25, 5.1, ph_z + 1.6),
             col_pent)
frame_3.data.materials.append(M_DARK_STEEL)

roof = box("Roof", (15.0, 12.0, 0.25), (0, 0, ph_z + 3.0), col_pent)
roof.data.materials.append(M_DARK_STEEL)
...
print("✅ Condominio industriale con materiali charcoal e rame - F12 per render")
