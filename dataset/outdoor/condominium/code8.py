...
def mat_concrete():
    ...
    ramp.color_ramp.elements[0].color = (0.82, 0.65, 0.52, 1)
    ramp.color_ramp.elements[1].color = (0.92, 0.75, 0.62, 1)
...
# CEMENTO SCURO TERRACOTTA
def mat_concrete_dark():
    m = bpy.data.materials.new("ConcreteDark")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
    ...
    ramp = nodes.new('ShaderNodeValToRGB')
    ramp.color_ramp.elements[0].color = (0.72, 0.55, 0.42, 1)
    ramp.color_ramp.elements[1].color = (0.82, 0.65, 0.52, 1)
...
def mat_glass():
    ...
    safe_set(bsdf.inputs, 'Alpha', 0.3)
...
# METALLO BRONZO SCURO
def mat_metal_railing():
    m = bpy.data.materials.new("MetalRailing")
    m.use_nodes = True
    nodes = m.node_tree.nodes
    links = m.node_tree.links
    nodes.clear()
...
for floor in range(P["floors"]):
    ...
    for i, x in enumerate(front_window_x):
        frame = box(f"WinFrame_F{floor}_F{i}",
                   ...
                   col_facade)
        frame.data.materials.append(M_RAILING)
...
for floor in range(P["floors"]):
    ...
    for side, x_sign in [("L", -1), ("R", 1)]:
        ...
        # Balcony with rounded edges
        bm = bmesh.new()
        bmesh.ops.create_cube(bm, size=1.0)
        bmesh.ops.bevel(bm, geom=bm.edges[:] + bm.verts[:], offset=0.15, segments=3)
        
        for v in bm.verts:
            v.co.x *= P["balcony_w"]
            v.co.y *= P["balcony_depth"]
            v.co.z *= 0.2
        
        mesh = bpy.data.meshes.new(f"Balcony_{side}_F{floor}")
        bm.to_mesh(mesh)
        bm.free()
        
        balcony = bpy.data.objects.new(f"Balcony_{side}_F{floor}", mesh)
        balcony.location = Vector((x_pos, y_pos, z_base + 0.1))
        col_balconies.objects.link(balcony)
...
print("✅ Condominium Mediterranean variation - F12 per render")
