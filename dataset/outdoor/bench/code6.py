...
def create_brushed_steel_material():
    ...
    bsdf = nodes.new('ShaderNodeBsdfPrincipled')
    bsdf.inputs['Base Color'].default_value = (0.7, 0.72, 0.75, 1)
...
steel_mat = create_brushed_steel_material()
...
# GAMBE - anteriori (acciaio spazzolato)
...
# GAMBE - posteriori (acciaio spazzolato)
for x in [-seat_width/2 + 0.1, seat_width/2 - 0.1]:
...
# BRACCIOLI - collegati allo schienale (acciaio spazzolato)
for x_sign in [-1, 1]:
    ...
    curve.data.materials.append(steel_mat)

# DECORAZIONI SCHIENALE (50% più grandi, acciaio spazzolato)
for i in range(7):
    x_pos = -0.6 + i * 0.2
    for j in range(3):
        z_pos = 0.7 + j * 0.15
        ...
        dec.data.materials.append(steel_mat)
...
print("✓ Modern bench with brushed steel finish created!")