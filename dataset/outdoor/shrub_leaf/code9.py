...
def aggiungi_ramo(pos_start, direzione, lunghezza, spessore_base, profondita=0, parent_idx=None):
    ...
    if profondita < 3:
        # Increase number of branches per level to 4-6
        num_ramificazioni = random.randint(4, 6) if profondita < 2 else random.randint(2, 4)
...
# Lighter reddish-brown tone
bsdf.inputs['Base Color'].default_value = (0.42, 0.28, 0.18, 1)
# Reduced roughness for smoother appearance
bsdf.inputs['Roughness'].default_value = 0.65
obj_rami.data.materials.append(mat_corteccia)

# Add leaves as icospheres
...
for pos in punti_foglie:
    for _ in range(random.randint(2, 4)):
        ...
        # Larger leaves (1.4x scale)
        leaf_scale = random.uniform(0.6, 1.0) * 1.4
        foglia.scale = (leaf_scale, leaf_scale, leaf_scale)
