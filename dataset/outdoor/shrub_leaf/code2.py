...
def aggiungi_ramo(pos_start, direzione, lunghezza, spessore_base, profondita=0, parent_idx=None):
    ...
    if profondita > 3 or random.random() < 0.2:
        return
    ...
    if profondita >= 2:
        for _ in range(random.randint(2, 4)):
            offset_foglia = Vector((
                random.uniform(-0.04, 0.04),
                random.uniform(-0.04, 0.04),
                random.uniform(-0.02, 0.04)
            ))
            punti_foglie.append(pos + offset_foglia)
...
bsdf.inputs['Base Color'].default_value = (0.52, 0.38, 0.22, 1)
bsdf.inputs['Roughness'].default_value = 0.75
obj_rami.data.materials.append(mat_corteccia)

# Create base leaf object with lower poly count
bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=1, radius=0.025, location=(0, 0, 0))
...
bsdf_f.inputs['Base Color'].default_value = (0.8, 0.35, 0.15, 1)
...
for pos in punti_foglie:
    ...
    foglia.location = pos
    foglia.scale = (random.uniform(0.4, 0.7),) * 3
...
print("Optimized autumn bush generated!")