...
# Base palo (più larga, nera metallica opaca)
...
bsdf_nero.inputs["Base Color"].default_value = (0.1, 0.1, 0.1, 1)
bsdf_nero.inputs["Metallic"].default_value = 0.8
bsdf_nero.inputs["Roughness"].default_value = 0.8
base.data.materials.append(mat_nero)

# Palo principale (nero metallico opaco, leggermente più spesso)
bpy.ops.mesh.primitive_cylinder_add(radius=0.05, depth=2.3, location=(0, 0.035, 1.25))
palo = bpy.context.active_object
palo.scale[0] = 0.9
palo.scale[1] = 0.9
mat_palo = bpy.data.materials.new("PaloNeroMetallo")
...
bsdf_palo.inputs["Roughness"].default_value = 0.85
palo.data.materials.append(mat_palo)

# Supporti orizzontali (2 staffe nere opache)
...
bsdf_bianco.inputs["Roughness"].default_value = 0.7
bordo.data.materials.append(mat_bianco)

# Segnale STOP arancione vibrante
...
bsdf_arancione.inputs["Roughness"].default_value = 0.3
...
mat_bianco_testo = bpy.data.materials.new("BiancoTesto")
mat_bianco_testo.use_nodes = True
bsdf_bianco_testo = mat_bianco_testo.node_tree.nodes["Principled BSDF"]
bsdf_bianco_testo.inputs["Base Color"].default_value = (1, 1, 1, 1)
bsdf_bianco_testo.inputs["Roughness"].default_value = 0.7
text_obj.data.materials.append(mat_bianco_testo)
