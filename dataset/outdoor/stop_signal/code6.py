...
# Base palo (più larga, nero metallico)
...
# Palo principale (nero metallico elegante, leggermente conico)
...
# Supporti orizzontali arrotondati (2 staffe rettangolari arrotondate)
for i, z_pos in enumerate([2.3, 2.5]):
    ...
    # Aggiungi modificatore per arrotondare
...
mat_rosso = bpy.data.materials.new("RossoStop")
mat_rosso.use_nodes = True
bsdf_rosso = mat_rosso.node_tree.nodes["Principled BSDF"]
bsdf_rosso.inputs["Base Color"].default_value = (0.8, 0.0, 0.0, 1)
bsdf_rosso.inputs["Roughness"].default_value = 0.2
stop_sign.data.materials.append(mat_rosso)
