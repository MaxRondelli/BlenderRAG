...
mat_marble = bpy.data.materials.new(name="DarkCharcoalMarble")
...
# Configura color ramp per colori marmo scuro carbone
color_ramp.color_ramp.elements[0].color = (0.25, 0.25, 0.25, 1.0)
...
# Configura BSDF per aspetto più rovinato
bsdf.inputs['Roughness'].default_value = 0.7

# MATERIALE ACQUA TURCHESE PROFONDO
mat_water = bpy.data.materials.new(name="DeepTurquoiseWater")
...
bsdf_water.inputs['Alpha'].default_value = 0.7
...
# ANELLO INTERMEDIO - più spesso del 50%
bpy.ops.mesh.primitive_cylinder_add(radius=1.9, depth=0.225, location=(0, 0, 0.3625))
mid_ring = bpy.context.active_object
mid_ring.name = "Mid_Ring"
mid_ring.data.materials.append(mat_marble)

# PIEDISTALLO CENTRALE
bpy.ops.mesh.primitive_cylinder_add(radius=0.9, depth=1.4, location=(0, 0, 1.1375))
pedestal = bpy.context.active_object
pedestal.name = "Pedestal"
pedestal.data.materials.append(mat_marble)

# VASCA PRINCIPALE - base più spessa del 50%
bpy.ops.mesh.primitive_cylinder_add(radius=1.8, depth=0.3, location=(0, 0, 1.9375))
...
bpy.ops.mesh.primitive_torus_add(major_radius=1.8, minor_radius=0.35, location=(0, 0, 2.0875))
...
# ACQUA nella vasca
bpy.ops.mesh.primitive_cylinder_add(radius=1.7, depth=0.05, location=(0, 0, 2.0375))
...
print("Fontana con texture marmo scuro carbone creata!")
