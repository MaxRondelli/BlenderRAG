...
# Deforma per forma irregolare PRIMA dei modifiers - più piatta e angolare
bpy.ops.object.mode_set(mode='EDIT')
bpy.ops.mesh.select_all(action='SELECT')
bpy.ops.transform.resize(value=(1.8, 1.2, 0.6))
...
# Displacement primario per forma grezza con spigoli - aumentato per più definizione
...
# Displacement secondario per parti taglienti
...
# Displacement terzo per micro-dettagli
...
# Materiale PBR con variazioni di colore più scure
...
# Colore base più scuro grigio-marrone
...
ramp_base.color_ramp.elements[0].color = (0.25, 0.22, 0.18, 1)  # Grigio-marrone molto scuro
ramp_base.color_ramp.elements[1].color = (0.35, 0.30, 0.25, 1)  # Grigio-marrone scuro

# Layer ruggine/ossidazione
noise_rust = nodes.new(type='ShaderNodeTexNoise')
noise_rust.inputs['Scale'].default_value = 2.5
...
ramp_rust.color_ramp.elements[0].color = (0.28, 0.18, 0.12, 1)  # Marrone scuro
ramp_rust.color_ramp.elements[1].position = 0.7
ramp_rust.color_ramp.elements[1].color = (0.52, 0.35, 0.22, 1)  # Ruggine
...
ramp_dark.color_ramp.elements[0].color = (0.15, 0.15, 0.13, 1)  # Quasi nero
ramp_dark.color_ramp.elements[1].position = 0.5
ramp_dark.color_ramp.elements[1].color = (0.35, 0.32, 0.28, 1)  # Grigio scuro

# Mix colori: base + ruggine
mix1 = nodes.new(type='ShaderNodeMix')
mix1.data_type = 'RGBA'


# Mix: precedente + macchie scure
...
print("Rock iperrealistico scuro e consumato generato!")
