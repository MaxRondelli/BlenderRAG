...
bsdf.inputs['Base Color'].default_value = (0.2, 0.1, 0.05, 1)
...
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (1.0, 0.8, 0.1, 1)
...
for i in range(num_petali):
    ...
    mod_simple.angle = random.uniform(0.5, 0.8)
    mod_simple.deform_axis = 'Y'
    
    petalo.data.materials.append(mat_petalo)

# Stelo più spesso e verde vibrante
bpy.ops.mesh.primitive_cylinder_add(radius=0.12*scale_factor, depth=2.5*scale_factor, location=(0, 0, -1.25*scale_factor), vertices=32)
...
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (0.1, 0.6, 0.15, 1)
