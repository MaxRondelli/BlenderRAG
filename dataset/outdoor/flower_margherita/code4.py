...
# Centro giallo - ora più grande e arancione
bpy.ops.mesh.primitive_uv_sphere_add(radius=0.5*scale_factor, location=(0, 0, 0), segments=64, ring_count=32)
...
# Petali - ora rosa con più rugosità
...
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (0.95, 0.7, 0.8, 1)
bsdf.inputs['Roughness'].default_value = 0.6

output = nodes.new('ShaderNodeOutputMaterial')
mat_petalo.node_tree.links.new(bsdf.outputs['BSDF'], output.inputs['Surface'])

num_petali = 13  # Ridotto da 21 a 13
...
# Stelo
bpy.ops.mesh.primitive_cylinder_add(radius=0.06*scale_factor, depth=2.5*scale_factor, location=(0, 0, -1.25*scale_factor), vertices=32)
...
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (0.15, 0.35, 0.1, 1)
bsdf.inputs['Roughness'].default_value = 0.6
