...
# Centro bruno-dorato più grande e scuro
...
bsdf.inputs['Base Color'].default_value = (0.4, 0.25, 0.05, 1)
...
# Petali gialli con punte arancioni e gradazione di colore
...
# Crea gradazione di colore dal giallo all'arancione
...
colorramp.color_ramp.elements[0].color = (1.0, 0.8, 0.1, 1.0)  # Giallo base
colorramp.color_ramp.elements[1].position = 1.0
colorramp.color_ramp.elements[1].color = (1.0, 0.4, 0.0, 1.0)  # Arancione punta

output = nodes.new('ShaderNodeOutputMaterial')

mat_petalo.node_tree.links.new(texcoord.outputs['Generated'], mapping.inputs['Vector'])
mat_petalo.node_tree.links.new(mapping.outputs['Vector'], gradient.inputs['Vector'])
...
for i in range(num_petali):
    ...
    dist = 0.62 * scale_factor + random.uniform(-0.02, 0.02) * scale_factor
    ...
    mod_simple.angle = random.uniform(0.2, 0.4)
    mod_simple.deform_axis = 'Y'
    
    petalo.data.materials.append(mat_petalo)

# Stelo più spesso con creste verticali
bpy.ops.mesh.primitive_cylinder_add(radius=0.1*scale_factor, depth=2.8*scale_factor, location=(0, 0, -1.4*scale_factor), vertices=16)
...
# Creste verticali con Wave texture
...
mat_stelo.node_tree.links.new(mix.outputs['Result'], bump.inputs['Height'])
