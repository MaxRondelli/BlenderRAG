...
# Centro marrone-arancione più scuro (sunflower center)
...
bsdf.inputs['Base Color'].default_value = (0.4, 0.25, 0.1, 1)  # Darker brown-orange
bsdf.inputs['Roughness'].default_value = 0.7

bump = nodes.new('ShaderNodeBump')
bump.inputs['Strength'].default_value = 1.5  # Increased bump intensity
...
# Petali giallo-dorati (sunflower petals)
...
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (0.95, 0.8, 0.15, 1)  # Golden-yellow
bsdf.inputs['Roughness'].default_value = 0.3

output = nodes.new('ShaderNodeOutputMaterial')
mat_petalo.node_tree.links.new(bsdf.outputs['BSDF'], output.inputs['Surface'])

num_petali = 15  # Reduced number of petals
for i in range(num_petali):
    ...
    petalo.scale = (0.18, 0.6, 1)  # Slightly wider petals
...
# Stelo più spesso e verde vibrante
bpy.ops.mesh.primitive_cylinder_add(radius=0.09*scale_factor, depth=2.5*scale_factor, location=(0, 0, -1.25*scale_factor), vertices=32)  # 50% thicker stem
...
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (0.1, 0.6, 0.15, 1)  # More vibrant green
bsdf.inputs['Roughness'].default_value = 0.8  # Higher roughness for natural texture
