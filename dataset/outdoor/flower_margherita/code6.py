...
bpy.ops.mesh.primitive_uv_sphere_add(radius=0.52*scale_factor, location=(0, 0, 0), segments=64, ring_count=32)
...
bsdf.inputs['Base Color'].default_value = (0.25, 0.15, 0.08, 1)
...
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (1.0, 0.85, 0.1, 1)
...
for i in range(num_petali):
    ...
    dist = 0.55 * scale_factor + random.uniform(-0.02, 0.02) * scale_factor
...
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (0.08, 0.25, 0.05, 1)
