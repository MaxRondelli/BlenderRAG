...
# Set up candle material (red wax color)
bsdf = candle_mat.node_tree.nodes["Principled BSDF"]
bsdf.inputs['Base Color'].default_value = (0.8, 0.15, 0.12, 1.0)  # Deep red color
bsdf.inputs['Roughness'].default_value = 0.3

# Create wax drip effect using small spheres
for i in range(3):
    angle = i * 120  # Space drips around the candle
    x_pos = 0.12 * math.cos(math.radians(angle))
    y_pos = 0.12 * math.sin(math.radians(angle))
    z_pos = 0.4 - (i * 0.08)
    ...
    drip.name = f"Wax_Drip_{i}"
    drip.scale[2] = 1.8  # Stretch vertically for drip effect
    
    # Create golden wax drip material
    drip_mat = bpy.data.materials.new(name=f"Drip_Material_{i}")
    drip_mat.use_nodes = True
    drip.data.materials.append(drip_mat)
    
    drip_bsdf = drip_mat.node_tree.nodes["Principled BSDF"]
    drip_bsdf.inputs['Base Color'].default_value = (0.9, 0.7, 0.2, 1.0)  # Golden color
    drip_bsdf.inputs['Metallic'].default_value = 0.2
    drip_bsdf.inputs['Roughness'].default_value = 0.25
...
emission.inputs['Color'].default_value = (0.8, 0.9, 1.0, 1.0)  # Blue-white flame
...
flame_light.data.color = (0.8, 0.9, 1.0)
...
ground_bsdf.inputs['Base Color'].default_value = (0.2, 0.2, 0.2, 1.0)
ground_bsdf.inputs['Roughness'].default_value = 0.6
