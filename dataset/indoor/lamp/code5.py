...
def create_brass_base():
    ...
    # Antique brass color with greenish patina
    node_bsdf.inputs['Base Color'].default_value = (0.6, 0.4, 0.2, 1.0)
...
def create_glass_bulb():
    ...
    # Amber glass color
    node_bsdf.inputs['Base Color'].default_value = (1.0, 0.6, 0.2, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 0.0
    node_bsdf.inputs['Roughness'].default_value = 0.1
    
    # Warm amber glow
    node_emission.inputs['Color'].default_value = (1.0, 0.7, 0.3, 1.0)
...
# Create the brass neck/stem between bulb and shade
def create_neck():
    ...
    # Use same brass material
    mat_brass = bpy.data.materials.get("Antique_Brass")
    if mat_brass:
        neck.data.materials.append(mat_brass)
    
    return neck

# Create the amber lampshade
def create_lampshade():
    ...
    # Create amber shade material
    mat_shade = bpy.data.materials.new(name="Amber_Shade")
    mat_shade.use_nodes = True
    shade.data.materials.append(mat_shade)
    
    nodes = mat_shade.node_tree.nodes
    nodes.clear()
    ...
    # Deep amber shade color
    node_bsdf.inputs['Base Color'].default_value = (0.9, 0.5, 0.1, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 0.0
    node_bsdf.inputs['Roughness'].default_value = 0.2
    
    # Warm amber emission
    node_emission.inputs['Color'].default_value = (1.0, 0.6, 0.2, 1.0)
    
    links = mat_shade.node_tree.links
    links.new(node_bsdf.outputs['BSDF'], node_mix.inputs[1])
    links.new(node_emission.outputs['Emission'], node_mix.inputs[2])
...
def create_lighting():
    ...
    # Rim light to highlight brass edges
...
print("Creating vintage brass lamp...")
