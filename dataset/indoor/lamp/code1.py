...
def create_brass_base():
    # Create a cone for the brass stand - slightly wider
    ...
    # Create brass material with golden tones
    ...
    # Rich brass/golden color
    node_bsdf.inputs['Base Color'].default_value = (0.8, 0.65, 0.3, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 0.9
    node_bsdf.inputs['Roughness'].default_value = 0.3
    
    # Noise texture for patina effect
    ...
    # Color ramp for brass variations
    node_colorramp.color_ramp.elements[0].color = (0.7, 0.55, 0.25, 1.0)
    node_colorramp.color_ramp.elements[1].color = (0.9, 0.75, 0.35, 1.0)
    
    # Mix brass base with texture variation
    node_mix.inputs['Factor'].default_value = 0.3
    node_mix.inputs[6].default_value = (0.8, 0.65, 0.3, 1.0)
    
    links = mat_base.node_tree.links
    links.new(node_noise.outputs['Fac'], node_colorramp.inputs['Fac'])
    links.new(node_colorramp.outputs['Color'], node_mix.inputs[7])
...
# Create the bulbous glass base with warmer tint
def create_glass_bulb():
    # Create UV sphere - slightly wider
    ...
    for vert in mesh.vertices:
        ...
        if z > 0.3:  # Upper portion - narrow neck
            ...
            scale_factor = 1.15 + math.sin((z + 0.2) * 3) * 0.1
        else:  # Bottom - rounded base
            scale_factor = 0.9 + (z + 0.2) * 0.3
        
        scale_factor = max(0.3, min(1.25, scale_factor))
    ...
    # Scale the bulb - wider proportions
    bulb.scale = (0.8, 0.8, 0.8)
    ...
    # Create warm tinted glass material
    mat_glass = bpy.data.materials.new(name="Warm_Glass")
    ...
    # Warm tinted glass appearance
    node_bsdf.inputs['Base Color'].default_value = (0.98, 0.92, 0.85, 1.0)
    ...
    node_emission.inputs['Strength'].default_value = 2.5
    ...
    node_mix.inputs[0].default_value = 0.35
...
def create_neck():
    ...
    # Use same warm glass material
    mat_glass = bpy.data.materials.get("Warm_Glass")
    if mat_glass:
...
def create_lampshade():
    # Create truncated cone for shade - wider proportions
    bpy.ops.mesh.primitive_cone_add(
        vertices=64,
        radius1=0.8,
        radius2=1.6,
    ...
    for vert in mesh.vertices:
        ...
        if z > 0:
            # Upper portion - flare outward more
            curve_factor = 1.0 + (z / 0.675) * 0.18
        else:
            # Lower portion - gentle inward curve
            curve_factor = 1.0 - abs(z / 0.675) * 0.1
...
print("Creating vintage brass lamp with warm tones...")
...
print("Vintage brass lamp created!")