...
def create_brass_base():
    # Create a cone for the brass stand
    ...
    # Create antique brass material
    mat_base = bpy.data.materials.new(name="Antique_Brass")
    ...
    # Antique brass color - warm golden bronze
    node_bsdf.inputs['Base Color'].default_value = (0.7, 0.5, 0.2, 1.0)
...
# Create the bulbous green glass base
def create_glass_bulb():
    ...
    # Scale the bulb slightly larger
    bulb.scale = (0.8, 0.8, 0.85)
    ...
    # Create emerald green glass material
    mat_glass = bpy.data.materials.new(name="Green_Glass")
    ...
    # Emerald green glass appearance
    node_bsdf.inputs['Base Color'].default_value = (0.15, 0.6, 0.3, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 0.0
    node_bsdf.inputs['Roughness'].default_value = 0.05
    
    # Warm green glow
    node_emission.inputs['Color'].default_value = (0.4, 0.8, 0.5, 1.0)
...
def create_neck():
    # Create cylinder for neck - slightly wider
    ...
    # Use same green glass material
    mat_glass = bpy.data.materials.get("Green_Glass")
    if mat_glass:
...
def create_lampshade():
    # Create truncated cone for shade - slightly larger
    bpy.ops.mesh.primitive_cone_add(
        vertices=64,
        radius1=0.8,
        radius2=1.6,
    ...
    for vert in mesh.vertices:
        ...
        if z > 0:
            ...
            curve_factor = 1.0 + (z / 0.7) * 0.2
        else:
            # Lower portion - gentle inward curve
            curve_factor = 1.0 - abs(z / 0.7) * 0.1
...
print("Creating vintage brass and green glass lamp...")
...
print("Vintage lamp created!")
print("- Brass base: Antique golden bronze finish")
print("- Glass shade and bulb: Emerald green with warm glow")