...
# Create the amber glass base with more angular shape
def create_glass_bulb():
    ...
    for vert in mesh.vertices:
        ...
        # Create more angular transitions
        if z > 0.4:  # Upper portion - sharper neck transition
            scale_factor = 0.3 + (1.0 - z) * 0.5
        elif z > -0.1:  # Middle - less rounded, more geometric
            scale_factor = 1.0 + math.sin((z + 0.1) * 4) * 0.15
        else:  # Bottom - more defined base
            scale_factor = 0.9 + (z + 0.1) * 0.2
        
        scale_factor = max(0.25, min(1.15, scale_factor))
    ...
    # Warm amber glow
    node_emission.inputs['Color'].default_value = (1.0, 0.65, 0.2, 1.0)
    ...
    node_mix.inputs[0].default_value = 0.4
...
# Create the neck/stem with increased thickness
def create_neck():
    # Create cylinder for neck with more substantial thickness
...
def create_lampshade():
    ...
    for vert in mesh.vertices:
        ...
        # Create more angular transitions instead of smooth curves
        if z > 0.2:
            # Upper portion - sharp outward angle
            curve_factor = 1.0 + (z / 0.625) * 0.25
        elif z > -0.2:
            # Middle transition - linear
            curve_factor = 1.0 + (z / 0.625) * 0.1
        else:
            # Lower portion - slight inward taper
            curve_factor = 1.0 - abs(z / 0.625) * 0.05
    ...
    # Add solidify modifier with increased thickness
...
print("Modern lamp created with amber glass and black metal finish!")
print("- Metal base: Matte black finish")
print("- Glass shade and bulb: Warm amber with increased glow")
