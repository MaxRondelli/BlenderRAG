...
def create_glass_bulb():
    ...
    for vert in mesh.vertices:
        ...
        # Create smooth bulbous shape - slightly wider
        if z > 0.3:  # Upper portion - narrow neck
            scale_factor = 0.4 + (1.0 - z) * 0.45
        elif z > -0.2:  # Middle - widest part
            scale_factor = 1.15 + math.sin((z + 0.2) * 3) * 0.12
        else:  # Bottom - rounded base
            scale_factor = 0.9 + (z + 0.2) * 0.35
        
        scale_factor = max(0.35, min(1.25, scale_factor))
    ...
    # Warm amber glass appearance
    node_bsdf.inputs['Base Color'].default_value = (0.95, 0.75, 0.4, 1.0)
...
def create_lampshade():
    # Create truncated cone for shade - slightly different proportions
    bpy.ops.mesh.primitive_cone_add(
        vertices=64,
        radius1=0.8,
        radius2=1.6,
...
def create_lighting():
    ...
    rim_light.data.color = (0.95, 0.85, 0.9)
...
print("Creating vintage brass and amber lamp...")
...
print("Vintage brass and amber lamp created!")
print("- Brass base: Warm golden metallic finish")
print("- Glass shade and bulb: Amber glass with warm golden glow")
