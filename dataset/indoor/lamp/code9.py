...
def create_brass_base():
    ...
    # Warm brass color - golden yellow with bronze tint
...
# Create the amber glass bulb
def create_amber_bulb():
    # Create UV sphere
    ...
    node_bsdf.inputs['Base Color'].default_value = (1.0, 0.7, 0.3, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 0.0
    node_bsdf.inputs['Roughness'].default_value = 0.1
    
    # Warm amber glow
    node_emission.inputs['Color'].default_value = (1.0, 0.8, 0.4, 1.0)
...
# Create the green glass lampshade
def create_lampshade():
    ...
    mat_green_glass = bpy.data.materials.new(name="Green_Glass")
    mat_green_glass.use_nodes = True
    shade.data.materials.append(mat_green_glass)
    
    nodes = mat_green_glass.node_tree.nodes
    nodes.clear()
    ...
    # Rich emerald green glass
    node_bsdf.inputs['Base Color'].default_value = (0.1, 0.6, 0.2, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 0.0
    node_bsdf.inputs['Roughness'].default_value = 0.05
    
    # Soft green glow
    node_emission.inputs['Color'].default_value = (0.3, 0.8, 0.4, 1.0)
    
    links = mat_green_glass.node_tree.links
    links.new(node_bsdf.outputs['BSDF'], node_mix.inputs[1])
    links.new(node_emission.outputs['Emission'], node_mix.inputs[2])
    node_mix.inputs[0].default_value = 0.2  # Mix factor
    links.new(node_mix.outputs['Shader'], node_output.inputs['Surface'])
...
print("Creating vintage brass lamp with green shade...")

# Create all components
base = create_brass_base()
bulb = create_amber_bulb()
neck = create_neck()
...
print("- Brass base and neck: Warm golden brass finish")
print("- Green glass shade: Rich emerald green")
print("- Amber bulb: Warm amber-tinted glass with glow")
