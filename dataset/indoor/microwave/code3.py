...
# --- MAIN BODY ---
# Outer shell - BLACK FINISH
outer_shell = create_box(
    "Microwave_Shell",
    (0, 0, micro_height/2),
    (micro_width, micro_depth, micro_height),
    (0.15, 0.15, 0.15, 1),  # Dark black/charcoal
    metallic=0.8,
    roughness=0.1
)
...
inner_cavity = create_box(
    ...
    (0.05, 0.05, 0.05, 1),  # Very dark interior
    metallic=0.0,
    roughness=0.9
)
...
door_frame = create_box(
    ...
    (0.12, 0.12, 0.12, 1),  # Matching black
    metallic=0.8,
    roughness=0.1
)
...
door_window = create_box(
    ...
    (0.02, 0.02, 0.02, 1),  # Darker window
    metallic=0.2,
    roughness=0.3
)

# Door handle (horizontal bar) - STAINLESS STEEL
...
door_handle = create_box(
    ...
    (0.8, 0.8, 0.85, 1),  # Stainless steel
    metallic=0.9,
    roughness=0.1
)

# Handle end caps (cylindrical) - STAINLESS STEEL
...
control_panel = create_box(
    ...
    (0.05, 0.05, 0.05, 1),  # Very dark black
    metallic=0.2,
    roughness=0.4
)
...
display_screen = create_box(
    ...
    (0.0, 0.1, 0.8, 1),  # Blue LCD display
    metallic=0.0,
    roughness=0.1
)

# --- NUMBER PAD (3x4 grid) - RED ACCENTS ---
button_size = 0.015
button_spacing = 0.022
button_depth = 0.008
button_start_z = display_z - display_height/2 - 0.03

# Create 12 buttons (1-9, 0, Start, Stop) - RED COLOR
button_positions = []
for row in range(4):
    for col in range(3):
...
for i, (x, z) in enumerate(button_positions):
    button = create_cylinder(
        ...
        button_size/2,
        button_depth,
        (0.8, 0.1, 0.1, 1),  # Bright red buttons
        metallic=0.3,
        roughness=0.4
    )
    button.rotation_euler = (math.pi/2, 0, 0)

# --- DOOR BUTTON (Opens door) - RED ACCENT ---
...
door_open_button = create_box(
    ...
    (0.9, 0.0, 0.0, 1),  # Bright red for "open" button
    metallic=0.4,
    roughness=0.3
)
...
bsdf.inputs['Base Color'].default_value = (1.0, 0.95, 0.8, 1)
...
for i, pos in enumerate(feet_positions):
    foot = create_cylinder(
        ...
        foot_radius,
        foot_height,
        (0.1, 0.1, 0.1, 1),  # Black rubber feet
        metallic=0.0,
        roughness=0.9
    )
...
print("BLACK MODERN MICROWAVE WITH RED ACCENTS GENERATED!")