...
# --- MAIN BODY ---
# Outer shell - Changed to black with high metallic finish
outer_shell = create_box(
    "Microwave_Shell",
    (0, 0, micro_height/2),
    (micro_width, micro_depth, micro_height),
    (0.1, 0.1, 0.12, 1),  # Dark blue-black
    metallic=0.8,
    roughness=0.1
)
...
# Door outer frame - Black to match body
door_frame = create_box(
    ...
    (0.08, 0.08, 0.1, 1),  # Dark blue-black
    metallic=0.8,
    roughness=0.15
)
...
door_window = create_box(
    ...
    (0.02, 0.02, 0.05, 1),  # Very dark with blue tint
    metallic=0.3,
    roughness=0.2
)

# Door handle - Changed to bright chrome finish
...
door_handle = create_box(
    ...
    (0.9, 0.9, 0.95, 1),  # Bright chrome
    metallic=0.95,
    roughness=0.05
)

# Handle end caps (cylindrical) - Chrome finish
...
control_panel = create_box(
    ...
    (0.05, 0.05, 0.08, 1),  # Very dark blue-black
    metallic=0.6,
    roughness=0.3
)

# --- DISPLAY SCREEN - Changed to blue LED
...
display_screen = create_box(
    ...
    (0.0, 0.2, 0.8, 1),  # Bright blue LED display
    metallic=0.0,
    roughness=0.0
)

# --- NUMBER PAD (3x4 grid) - Darker buttons with metallic finish
button_size = 0.015
...
for i, (x, z) in enumerate(button_positions):
    button = create_cylinder(
        ...
        button_size/2,
        button_depth,
        (0.15, 0.15, 0.2, 1),  # Dark metallic buttons
        metallic=0.7,
        roughness=0.3
    )
...
door_open_button = create_box(
    ...
    (0.8, 0.15, 0.15, 1),  # Bright red button
    metallic=0.6,
    roughness=0.2
)
...
turntable = create_cylinder(
    ...
    turntable_radius,
    turntable_thickness,
    (0.75, 0.8, 0.85, 1),  # Slightly blue-tinted glass
    metallic=0.0,
    roughness=0.05
)
...
for i in range(3):
    ...
    roller = create_cylinder(
        ...
        roller_radius,
        roller_height,
        (0.3, 0.3, 0.35, 1),  # Dark metallic
        metallic=0.7,
        roughness=0.4
    )

# --- VENTILATION GRILLES (Top and Side) - Darker metallic
# Top vent
...
top_vent = create_box(
    ...
    (0.1, 0.1, 0.15, 1),  # Dark blue-black
    metallic=0.8,
    roughness=0.2
)
...
cord_1 = create_cylinder(
    ...
    cord_radius,
    0.05,
    (0.05, 0.05, 0.08, 1),  # Very dark blue-black cord
    metallic=0.0,
    roughness=0.8
)
...
plug = create_box(
    ...
    (0.03, 0.015, 0.01),
    (0.1, 0.1, 0.12, 1),  # Dark metallic plug
    metallic=0.5,
    roughness=0.4
)
...
brand_logo = create_box(
    ...
    (0.08, 0.002, 0.012),
    (0.8, 0.8, 0.9, 1),  # Chrome logo
    metallic=0.9,
    roughness=0.1
)

# --- INTERIOR LIGHT (inside cavity) - Blue LED
light_width = 0.03
light_height = 0.01
light_depth = 0.01
...
interior_light = create_box(
    ...
    (0.7, 0.85, 1.0, 1),  # Cool blue-white light
    metallic=0.0,
    roughness=0.0
)
...
for i, pos in enumerate(feet_positions):
    foot = create_cylinder(
        ...
        foot_radius,
        foot_height,
        (0.08, 0.08, 0.12, 1),  # Dark blue-black feet
        metallic=0.2,
        roughness=0.9
    )
...
print("=" * 60)
print("\nComponents created:")
print("  ✓ Black metallic outer shell")
print("  ✓ Chrome handle finish")
print("  ✓ Blue LED display and interior light")
print("  ✓ Enhanced metallic properties throughout")
print("=" * 60)