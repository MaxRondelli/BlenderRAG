...
# --- MAIN BODY ---
# Outer shell - Retro cream color
outer_shell = create_box(
    "Microwave_Shell",
    (0, 0, micro_height/2),
    (micro_width, micro_depth, micro_height),
    (0.95, 0.92, 0.85, 1),  # Cream/beige color
    metallic=0.1,
    roughness=0.3
)
...
inner_cavity = create_box(
    ...
    (0.15, 0.12, 0.08, 1),  # Warmer brown interior
    metallic=0.0,
    roughness=0.9
)
...
door_frame = create_box(
    ...
    (0.92, 0.89, 0.82, 1),  # Slightly darker cream
    metallic=0.1,
    roughness=0.3
)
...
door_window = create_box(
    ...
    (0.08, 0.06, 0.04, 1),  # Brownish mesh
    metallic=0.3,
    roughness=0.4
)
...
control_panel = create_box(
    ...
    (0.25, 0.22, 0.18, 1),  # Warm brown control panel
    metallic=0.05,
    roughness=0.6
)
...
display_screen = create_box(
    ...
    (0.8, 0.4, 0.1, 1),  # Amber display color
    metallic=0.0,
    roughness=0.1
)
...
# Create 12 buttons (1-9, 0, Start, Stop) - Brass colored
button_positions = []
for row in range(4):
    for col in range(3):
...
for i, (x, z) in enumerate(button_positions):
    button = create_cylinder(
        ...
        button_size/2,
        button_depth,
        (0.75, 0.58, 0.22, 1),  # Brass buttons
        metallic=0.7,
        roughness=0.3
    )
...
door_open_button = create_box(
    ...
    (0.8, 0.25, 0.15, 1),  # Warm orange-red color for "open" button
    metallic=0.2,
    roughness=0.4
)
...
turntable = create_cylinder(
    ...
    turntable_radius,
    turntable_thickness,
    (0.9, 0.88, 0.82, 1),  # Cream-tinted glass
    metallic=0.0,
    roughness=0.1
)

# Turntable support hub (center) - Brass colored
hub_radius = 0.015
hub_height = 0.02
hub_z = 0.025
...
turntable_hub = create_cylinder(
    ...
    hub_radius,
    hub_height,
    (0.75, 0.58, 0.22, 1),  # Brass hub
    metallic=0.7,
    roughness=0.3
)

# Turntable roller wheels (3 small wheels supporting the plate) - Brass colored
roller_radius = 0.008
roller_height = 0.008
...
for i in range(3):
    ...
    roller = create_cylinder(
        ...
        roller_radius,
        roller_height,
        (0.65, 0.48, 0.18, 1),  # Darker brass rollers
        metallic=0.6,
        roughness=0.4
    )

# --- VENTILATION GRILLES (Top and Side) ---
# Top vent - Brass colored
...
# Cord segment 1 (coming out of microwave) - Darker retro cord
cord_1 = create_cylinder(
    "Power_Cord_1",
    (cord_exit_x, cord_exit_y + 0.025, cord_exit_z),
    cord_radius,
    0.05,
    (0.2, 0.15, 0.1, 1),  # Brown cord
    metallic=0.0,
    roughness=0.8
)
cord_1.rotation_euler = (math.pi/2, 0, 0)

# Cord segment 2 (going down) - Darker retro cord
...
# Cord segment 3 (along surface) - Darker retro cord
...
# Power plug - Brass colored
plug = create_box(
    "Power_Plug",
    (cord_exit_x - 0.05, cord_exit_y + 0.05, 0.005),
    (0.03, 0.015, 0.01),
    (0.65, 0.48, 0.18, 1),  # Brass plug
    metallic=0.7,
    roughness=0.4
)
...
interior_light = create_box(
    ...
    (1.0, 0.85, 0.6, 1),  # Warmer amber light
    metallic=0.0,
    roughness=0.0
)
...
for i, pos in enumerate(feet_positions):
    foot = create_cylinder(
        ...
        foot_radius,
        foot_height,
        (0.25, 0.2, 0.15, 1),  # Brown rubber feet
        metallic=0.0,
        roughness=0.9
    )
...
print("=" * 60)
print("RETRO MICROWAVE GENERATED SUCCESSFULLY!")
print("=" * 60)
print("\nComponents created:")
print("  ✓ Outer shell with cream/beige color scheme")
print("  ✓ Inner cooking cavity with warm brown interior")
print("  ✓ Door with window and brass handle")
print("  ✓ Control panel with amber display and brass buttons")
print("  ✓ Turntable with brass support system")
print("  ✓ Brass ventilation grilles")
print("  ✓ Warm amber interior light")
print("  ✓ Brown power cord and brass plug")
print("  ✓ Brown rubber feet")
print("  ✓ Brass brand logo")
print("\nRetro styling applied with warm color palette!")
print("=" * 60)