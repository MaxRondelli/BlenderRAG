...
# --- MAIN BODY ---
# Outer shell - Vintage cream color
outer_shell = create_box(
    "Microwave_Shell",
    (0, 0, micro_height/2),
    (micro_width, micro_depth, micro_height),
    (0.92, 0.88, 0.82, 1),  # Warm cream/beige
    metallic=0.1,
    roughness=0.3
)
...
inner_cavity = create_box(
    ...
    (0.15, 0.12, 0.1, 1),  # Warmer dark interior
    metallic=0.0,
    roughness=0.8
)
...
door_frame = create_box(
    ...
    (0.90, 0.86, 0.80, 1),  # Slightly darker cream
    metallic=0.1,
    roughness=0.3
)
...
door_window = create_box(
    ...
    (0.1, 0.08, 0.06, 1),  # Brownish tinted glass
    metallic=0.0,
    roughness=0.2
)

# Door handle (horizontal bar) - Bronze finish
...
door_handle = create_box(
    ...
    (0.65, 0.45, 0.25, 1),  # Bronze color
    metallic=0.8,
    roughness=0.2
)

# Handle end caps (cylindrical) - Bronze
...
# Position control panel at the same Y position as the door (front face)
panel_width = 0.12
...
control_panel = create_box(
    ...
    (0.25, 0.20, 0.15, 1),  # Warm brown control panel
    metallic=0.05,
    roughness=0.6
)
...
display_height = 0.04
# Position display in front of the control panel
...
display_screen = create_box(
    ...
    (0.1, 0.15, 0.05, 1),  # Amber vintage display
    metallic=0.0,
    roughness=0.1
)
...
display_mat.node_tree.nodes["Principled BSDF"].inputs['Emission Strength'].default_value = 2.0
...
# Create 12 buttons (1-9, 0, Start, Stop) - Bronze buttons
button_positions = []
for row in range(4):
    for col in range(3):
...
for i, (x, z) in enumerate(button_positions):
    button = create_cylinder(
        ...
        button_size/2,
        button_depth,
        (0.65, 0.45, 0.25, 1),  # Bronze buttons
        metallic=0.7,
        roughness=0.3
    )
...
door_open_button = create_box(
    ...
    (0.7, 0.25, 0.15, 1),  # Burnt orange color for "open" button
    metallic=0.4,
    roughness=0.4
)
...
turntable = create_cylinder(
    ...
    turntable_radius,
    turntable_thickness,
    (0.95, 0.92, 0.88, 1),  # Cream-tinted glass
    metallic=0.0,
    roughness=0.05
)

# Turntable support hub (center) - Bronze
hub_radius = 0.015
hub_height = 0.02
hub_z = 0.025
...
turntable_hub = create_cylinder(
    ...
    hub_radius,
    hub_height,
    (0.65, 0.45, 0.25, 1),  # Bronze hub
    metallic=0.8,
    roughness=0.3
)

# Turntable roller wheels (3 small wheels supporting the plate) - Bronze
roller_radius = 0.008
roller_height = 0.008
...
for i in range(3):
    ...
    roller = create_cylinder(
        ...
        roller_radius,
        roller_height,
        (0.65, 0.45, 0.25, 1),  # Bronze rollers
        metallic=0.7,
        roughness=0.4
    )
...
top_vent = create_box(
    ...
    (0.65, 0.45, 0.25, 1),  # Bronze vents
    metallic=0.8,
    roughness=0.3
)

# Side vent (right side, near back) - Bronze
...
# Cord segment 1 (coming out of microwave) - Brown cord
cord_1 = create_cylinder(
    "Power_Cord_1",
    (cord_exit_x, cord_exit_y + 0.025, cord_exit_z),
    cord_radius,
    0.05,
    (0.25, 0.15, 0.08, 1),  # Brown cord
    metallic=0.0,
    roughness=0.9
)
...
interior_light = create_box(
    ...
    (1.0, 0.9, 0.7, 1),  # Warmer white light
    metallic=0.0,
    roughness=0.0
)
...
light_mat.node_tree.nodes["Principled BSDF"].inputs['Emission Strength'].default_value = 4.0
...
for i, pos in enumerate(feet_positions):
    foot = create_cylinder(
        ...
        foot_radius,
        foot_height,
        (0.20, 0.15, 0.10, 1),  # Brown rubber feet
        metallic=0.0,
        roughness=0.95
    )

print("VINTAGE MICROWAVE GENERATED SUCCESSFULLY!")