...
outer_shell = create_box(
    ...
    (0.05, 0.05, 0.05, 1),  # Black color
    metallic=0.4,
    roughness=0.15
)

# Inner cavity - changed to darker charcoal
...
inner_cavity = create_box(
    ...
    (0.15, 0.15, 0.18, 1),  # Darker charcoal interior
    metallic=0.0,
    roughness=0.8
)
...
# Door outer frame - matching black with more metallic finish
...
# Door handle - changed to metallic silver
...
door_handle = create_box(
    ...
    (0.8, 0.8, 0.85, 1),  # Silver metallic
    metallic=0.9,
    roughness=0.1
)

# Handle end caps (cylindrical) - matching silver
...
control_panel = create_box(
    ...
    (0.08, 0.08, 0.08, 1),  # Matching black
    metallic=0.3,
    roughness=0.3
)
...
display_screen = create_box(
    ...
    (0.0, 0.2, 0.8, 1),  # Bright blue LCD display
    metallic=0.0,
    roughness=0.05
)
...
# Create 12 buttons - changed to silver
button_positions = []
for row in range(4):
    for col in range(3):
...
door_open_button = create_box(
    ...
    (0.7, 0.7, 0.75, 1),  # Silver to match other buttons
    metallic=0.7,
    roughness=0.3
)
...
# --- VENTILATION GRILLES (Top and Side) - Silver accents
# Top vent
...
# --- BRAND LOGO (embossed on door) - Silver
...
interior_light = create_box(
    ...
    (1.0, 1.0, 1.0, 1),  # Bright white light
    metallic=0.0,
    roughness=0.0
)
...
bsdf = light_mat.node_tree.nodes["Principled BSDF"]
...
print("=" * 60)
print("BLACK MICROWAVE WITH SILVER ACCENTS GENERATED SUCCESSFULLY!")
print("=" * 60)