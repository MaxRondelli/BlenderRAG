...
# --- MAIN BODY ---
# Outer shell - Changed to black with metallic finish
outer_shell = create_box(
    "Microwave_Shell",
    (0, 0, micro_height/2),
    (micro_width, micro_depth, micro_height),
    (0.08, 0.08, 0.08, 1),  # Dark black
    metallic=0.7,
    roughness=0.1
)
...
inner_cavity = create_box(
    ...
    (0.05, 0.05, 0.05, 1),  # Darker interior
    metallic=0.0,
    roughness=0.9
)
...
# Door outer frame - Changed to metallic silver
door_frame = create_box(
    ...
    (0.75, 0.75, 0.78, 1),  # Metallic silver
    metallic=0.9,
    roughness=0.1
)
...
# Door handle (horizontal bar) - Changed to bright silver
...
door_handle = create_box(
    ...
    (0.85, 0.85, 0.88, 1),  # Bright silver
    metallic=0.95,
    roughness=0.1
)

# Handle end caps (cylindrical) - Changed to bright silver
...
control_panel = create_box(
    ...
    (0.05, 0.05, 0.05, 1),  # Even darker black
    metallic=0.2,
    roughness=0.3
)
...
display_screen = create_box(
    ...
    (0.0, 0.5, 0.8, 1),  # Blue LCD display
    metallic=0.0,
    roughness=0.1
)
...
# Create 12 buttons (1-9, 0, Start, Stop) - Changed to darker color
button_positions = []
for row in range(4):
    for col in range(3):
...
for i, (x, z) in enumerate(button_positions):
    button = create_cylinder(
        ...
        button_size/2,
        button_depth,
        (0.15, 0.15, 0.15, 1),  # Darker buttons
        metallic=0.6,
        roughness=0.4
    )
...
door_open_button = create_box(
    ...
    (0.8, 0.2, 0.2, 1),  # Brighter red for contrast
    metallic=0.4,
    roughness=0.3
)
...
turntable = create_cylinder(
    ...
    turntable_radius,
    turntable_thickness,
    (0.9, 0.9, 0.95, 1),  # Slightly bluer glass
    metallic=0.0,
    roughness=0.05
)
...
for i in range(3):
    ...
    roller = create_cylinder(
        ...
        roller_radius,
        roller_height,
        (0.3, 0.3, 0.3, 1),  # Darker rollers
        metallic=0.5,
        roughness=0.4
    )

# --- VENTILATION GRILLES (Top and Side) ---
# Top vent - Changed to silver metallic
...
top_vent = create_box(
    ...
    (0.7, 0.7, 0.75, 1),  # Silver metallic
    metallic=0.8,
    roughness=0.2
)

# Side vent (right side, near back) - Changed to silver metallic
...
brand_logo = create_box(
    ...
    (0.08, 0.002, 0.012),
    (0.8, 0.8, 0.85, 1),  # Bright silver logo
    metallic=0.9,
    roughness=0.1
)
...
interior_light = create_box(
    ...
    (0.95, 0.95, 1.0, 1),  # Cooler white light
    metallic=0.0,
    roughness=0.0
)
...
light_mat.node_tree.nodes["Principled BSDF"].inputs['Base Color'].default_value = (0.95, 0.95, 1.0, 1)
...
print("=" * 60)
print("BLACK METALLIC MICROWAVE GENERATED SUCCESSFULLY!")
print("=" * 60)
print("\nComponents created:")
print("  ✓ Black outer shell with metallic finish")
print("  ✓ Inner cooking cavity")
print("  ✓ Silver metallic door with window and handle")
print("  ✓ Dark control panel with blue display and metallic buttons")
print("  ✓ Turntable with support system")
print("  ✓ Silver metallic ventilation grilles")
print("  ✓ Cool white interior light")
print("  ✓ Power cord and plug")
print("  ✓ Dark rubber feet")
print("  ✓ Silver brand logo")
print("\nVARIATION CHANGES:")
print("  • Main body changed to sleek black with high metallic finish")
print("  • Door and accents in bright metallic silver")
print("  • Buttons and details in darker industrial colors")
print("\nAll components are properly positioned in 3D space!")
print("=" * 60)