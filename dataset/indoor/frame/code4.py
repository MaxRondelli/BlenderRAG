...
def create_photo_frame(
    ...
    frame_depth=0.05,     # Depth/thickness of frame
    frame_width=0.06,     # Width of frame border (increased from 0.04)
    bevel_segments=2,     # Number of stepped bevels (reduced from 3)
    material_color=(0.35, 0.15, 0.08, 1.0)  # Dark mahogany color
):
    """
    ...
    # Create material for wood frame
    ...
    bsdf.inputs['Roughness'].default_value = 0.6  # Increased roughness for matte mahogany
...
if __name__ == "__main__":
    ...
    frame, backing, profile = create_photo_frame(
        ...
        outer_height=0.4,
        frame_depth=0.05,
        frame_width=0.06,  # Wider border
        bevel_segments=2,  # Fewer bevels
        material_color=(0.35, 0.15, 0.08, 1.0)  # Dark mahogany wood color
    )
