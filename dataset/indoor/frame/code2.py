...
def create_photo_frame(
    ...
    frame_depth=0.08,     # Depth/thickness of frame (increased)
    frame_width=0.05,     # Width of frame border (slightly wider)
    bevel_segments=4,     # Number of stepped bevels (increased)
    material_color=(0.3, 0.15, 0.08, 1.0),  # Dark mahogany color
    corner_radius=0.02    # Rounded corner radius
):
    """
    Create a decorative photo frame with stepped profile and rounded corners in Blender
    ...
    # Define the stepped profile points with more ornate shape
    profile_points = []
    
    # Start from front edge
    profile_points.append((0, 0))
    
    # Create stepped bevel profile with curved transitions
    step_depth = frame_depth / (bevel_segments + 1)
    step_width = frame_width / (bevel_segments + 1)
    
    for i in range(bevel_segments):
        # Step back with slight curve
        profile_points.append((step_width * (i + 0.3), step_depth * i))
        # Step in
        profile_points.append((step_width * (i + 0.7), step_depth * (i + 0.2)))
        # Step down
    ...
    profile_points.append((frame_width, frame_depth - 0.008))
    profile_points.append((0, frame_depth - 0.008))
    profile_points.append((0, 0))
    ...
    # Create the frame path (rounded rectangle for the frame outline)
    path_data = bpy.data.curves.new(name="FramePath", type='CURVE')
    path_data.dimensions = '3D'
    
    # Create rounded rectangular path
    path_spline = path_data.splines.new('BEZIER')
    
    # Define rounded rectangle points (centered at origin)
    half_w = outer_width / 2
    half_h = outer_height / 2
    cr = corner_radius
    
    # Create bezier points for rounded corners
    path_points = [
        # Top edge
        (-half_w + cr, half_h, 0),
        (half_w - cr, half_h, 0),
    ...
    path_spline.bezier_points.add(len(path_points) - 1)
    for i, point in enumerate(path_points):
        bp = path_spline.bezier_points[i]
    ...
    # Create material for mahogany wood frame
    ...
    color_ramp.color_ramp.elements[0].color = (0.2, 0.1, 0.05, 1.0)
    color_ramp.color_ramp.elements[1].color = (0.4, 0.2, 0.1, 1.0)
    ...
    backing_bsdf.inputs['Base Color'].default_value = (0.95, 0.95, 0.9, 1.0)
    ...
    print(f"Ornate photo frame created successfully!")
...
if __name__ == "__main__":
    ...
    frame, backing, profile = create_photo_frame(
        ...
        frame_width=0.05,
        bevel_segments=4,
        material_color=(0.3, 0.15, 0.08, 1.0),
        corner_radius=0.02
    )
