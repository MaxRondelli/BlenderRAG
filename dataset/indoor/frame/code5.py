...
def create_photo_frame(
    ...
    frame_depth=0.03,     # Depth/thickness of frame (reduced for sleeker look)
    frame_width=0.03,     # Width of frame border (reduced)
    bevel_segments=2,     # Number of stepped bevels (reduced for cleaner look)
    material_color=(0.8, 0.8, 0.85, 1.0)  # Metallic chrome color (RGBA)
):
    """
    ...
    # Create material for metallic chrome frame
    ...
    # Create backing material (darker)
    ...
    backing_bsdf.inputs['Base Color'].default_value = (0.2, 0.2, 0.2, 1.0)
...
if __name__ == "__main__":
    ...
    frame, backing, profile = create_photo_frame(
        ...
        frame_width=0.03,
        bevel_segments=2,
        material_color=(0.8, 0.8, 0.85, 1.0)  # Chrome metallic color
    )
