...
radius = 0.18          # 18cm radius (wider)
height = 0.35          # 35cm height
wall_thickness = 0.004 # 4mm wall thickness (thicker)
lid_height = 0.03      # 3cm lid height
...
# Create gunmetal material for body
gunmetal_mat = bpy.data.materials.new(name="Gunmetal_Steel")
gunmetal_mat.use_nodes = True
nodes = gunmetal_mat.node_tree.nodes
links = gunmetal_mat.node_tree.links
nodes.clear()

# Principled BSDF for metal
bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
bsdf.location = (0, 0)
bsdf.inputs['Base Color'].default_value = (0.35, 0.4, 0.45, 1.0)  # Darker gunmetal
bsdf.inputs['Metallic'].default_value = 1.0
bsdf.inputs['Roughness'].default_value = 0.4  # More textured finish
...
trash_can_body.data.materials.append(gunmetal_mat)
lid.data.materials.append(gunmetal_mat)

# Create charcoal plastic material for pedal
charcoal_mat = bpy.data.materials.new(name="Charcoal_Plastic")
charcoal_mat.use_nodes = True
plastic_nodes = charcoal_mat.node_tree.nodes
plastic_nodes.clear()

plastic_bsdf = plastic_nodes.new(type='ShaderNodeBsdfPrincipled')
plastic_bsdf.location = (0, 0)
plastic_bsdf.inputs['Base Color'].default_value = (0.15, 0.15, 0.15, 1.0)  # Charcoal gray
...
charcoal_mat.node_tree.links.new(plastic_bsdf.outputs['BSDF'], plastic_output.inputs['Surface'])

pedal_assembly.data.materials.append(charcoal_mat)
...
print("=" * 60)
print("INDUSTRIAL TRASH CAN GENERATED SUCCESSFULLY!")
print("=" * 60)
print(f"\nDimensions:")
...
print(f"  • Industrial gunmetal finish")
print("=" * 60)