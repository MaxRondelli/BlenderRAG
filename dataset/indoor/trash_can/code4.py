...
# Trash can dimensions - modified for variation
radius = 0.18          # 18cm radius (increased)
height = 0.30          # 30cm height (reduced)
wall_thickness = 0.004 # 4mm wall thickness (doubled)
lid_height = 0.025     # 2.5cm lid height (reduced)
...
# Create lid handle/knob - smaller size
bpy.ops.mesh.primitive_cylinder_add(
    vertices=32,
    radius=0.010,  # Reduced from 0.015
    depth=0.025,   # Reduced from 0.04
    location=(0, radius * 0.7, height + lid_height + 0.015)
)
...
# Create black metallic material for body - variation change
black_metal_mat = bpy.data.materials.new(name="Black_Metal")
black_metal_mat.use_nodes = True
nodes = black_metal_mat.node_tree.nodes
links = black_metal_mat.node_tree.links
nodes.clear()

# Principled BSDF for black metal
bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
bsdf.location = (0, 0)
bsdf.inputs['Base Color'].default_value = (0.05, 0.05, 0.08, 1.0)  # Dark metallic black
bsdf.inputs['Metallic'].default_value = 1.0
bsdf.inputs['Roughness'].default_value = 0.15  # Slightly more polished
...
trash_can_body.data.materials.append(black_metal_mat)
lid.data.materials.append(black_metal_mat)
...
print(f"  • Lid with compact handle")
print(f"  • Foot pedal mechanism")
print(f"  • Black metallic finish")
print("=" * 60)