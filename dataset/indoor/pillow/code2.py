...
def create_pillow_material(name="PillowMaterial", base_color=(0.4, 0.1, 0.15)):
    ...
    node_mapping.inputs['Scale'].default_value = (8.0, 8.0, 8.0)
    
    # Create checker texture for embossed pattern
    node_checker = nodes.new(type='ShaderNodeTexChecker')
    node_checker.location = (-400, 0)
    node_checker.inputs['Scale'].default_value = 4.0
    
    # Create ColorRamp for pattern control
    node_colorramp = nodes.new(type='ShaderNodeValToRGB')
    node_colorramp.location = (-200, 0)
    node_colorramp.color_ramp.elements[0].position = 0.3
    node_colorramp.color_ramp.elements[1].position = 0.7
    ...
    # Link nodes
    mat.node_tree.links.new(node_texcoord.outputs['Generated'], node_mapping.inputs['Vector'])
    mat.node_tree.links.new(node_mapping.outputs['Vector'], node_checker.inputs['Vector'])
    mat.node_tree.links.new(node_checker.outputs['Fac'], node_colorramp.inputs['Fac'])
    mat.node_tree.links.new(node_colorramp.outputs['Color'], node_bsdf.inputs['Normal'])
    mat.node_tree.links.new(node_bsdf.outputs['BSDF'], node_output.inputs['Surface'])
    
    return mat

def create_pillow(location=(0, 0, 0), size=(1.0, 1.0, 0.18), color=(0.4, 0.1, 0.15)):
    """
    ...
    simple_deform.angle = math.radians(3)
    simple_deform.deform_axis = 'X'
    
    # Add Displace modifier for embossed texture effect
    ...
    cloth.settings.uniform_pressure_force = 4
...
def setup_scene():
    ...
    bpy.ops.mesh.primitive_plane_add(location=(0, 0, -0.1), scale=(3, 3, 1))
