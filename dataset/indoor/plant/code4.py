...
# Materials
def create_modern_ceramic_material():
    """Create a modern black ceramic pot material with metallic accents"""
    mat = bpy.data.materials.new(name="ModernCeramic")
    mat.use_nodes = True
    ...
    node_bsdf.inputs['Base Color'].default_value = (0.08, 0.08, 0.12, 1.0)  # Deep charcoal black
...
# Create materials
ceramic_mat = create_modern_ceramic_material()
...
# Create leaves arranged in a denser spiral pattern
leaves = []
stem_top_z = pot_height + soil_height + stem_height

for i in range(num_leaves):
    # Calculate position along stem (leaves at different heights)
    height_ratio = 0.3 + (i / num_leaves) * 0.7  # Leaves in upper 70% of stem for denser look
    leaf_z = pot_height + soil_height + stem_height * height_ratio
    
    # Spiral angle with tighter spacing
    angle = (i / num_leaves) * math.pi * 4.5  # More rotations for denser arrangement
...
collection = bpy.data.collections.new("Modern_Potted_Plant")
...
print("Modern potted plant created successfully!")
