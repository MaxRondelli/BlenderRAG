...
def create_copper_material():
    """Create a metallic copper pot material"""
    mat = bpy.data.materials.new(name="Copper")
    mat.use_nodes = True
    ...
    node_bsdf.inputs['Base Color'].default_value = (0.95, 0.64, 0.54, 1.0)  # Copper color
...
pot.data.materials.append(copper_mat)
...
pot_rim.data.materials.append(copper_mat)
...
for i in range(num_leaves):
    ...
    height_ratio = 0.2 + (i / num_leaves) * 0.8  # Leaves in upper 80% of stem
    leaf_z = pot_height + soil_height + stem_height * height_ratio
    
    # Spiral angle
    angle = (i / num_leaves) * math.pi * 5  # Multiple rotations for more leaves
    ...
    # Scale to create leaf shape with slight variation
    size_variation = random.uniform(0.8, 1.2)
    leaf.dimensions = (leaf_width * size_variation, leaf_length * size_variation, 0.01)
    ...
    leaf.rotation_euler = (
        math.radians(60 + random.uniform(-15, 15)),  # Tilt up with more variation
        0,
        angle + math.radians(random.uniform(-20, 20))  # Face outward with variation
    )
...
print("Copper potted plant created successfully!")
