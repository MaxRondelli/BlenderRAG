...
def create_seat_cushion(width, depth, height, location, name):
    ...
    # Enhanced bevel for luxury leather cushion
    edges_to_bevel = [e for e in bm.edges]
    bmesh.ops.bevel(bm, geom=edges_to_bevel, offset=0.10, segments=5, 
...
def create_couch_section(width, depth, base_height, back_height, location, section_name, num_cushions=3):
    ...
    back_cushion_height = back_height - base_height + 0.18
...
def create_realistic_couch():
    """Generate a realistic luxury leather couch"""
    
    # Main couch dimensions (slightly taller proportions)
    main_width = 5.0
    main_depth = 1.8
    base_height = 0.48
    back_height = 1.8
    ...
    print("Luxury leather couch generated successfully!")

def create_couch_materials():
    """Create realistic luxury leather materials for the couch"""
    
    # Main leather material - rich brown leather
    ...
    # Color ramp for leather variation
    ramp = nodes.new(type='ShaderNodeValToRGB')
    ramp.location = (-100, 0)
    ramp.color_ramp.elements[0].color = (0.15, 0.10, 0.06, 1.0)
    ramp.color_ramp.elements[1].color = (0.25, 0.18, 0.12, 1.0)
    
    # Rich brown leather base color
    principled.inputs['Base Color'].default_value = (0.22, 0.15, 0.09, 1.0)
    principled.inputs['Roughness'].default_value = 0.3
    principled.inputs['IOR'].default_value = 1.4
    principled.inputs['Metallic'].default_value = 0.0
    
    # Connect nodes
    main_leather.node_tree.links.new(noise.outputs['Fac'], ramp.inputs['Fac'])
    main_leather.node_tree.links.new(ramp.outputs['Color'], principled.inputs['Base Color'])
    main_leather.node_tree.links.new(principled.outputs['BSDF'], output.inputs['Surface'])
    
    # Cushion leather material - slightly darker brown
    ...
    # Color ramp for cushion leather variation
    ramp = nodes.new(type='ShaderNodeValToRGB')
    ramp.location = (-100, 0)
    ramp.color_ramp.elements[0].color = (0.12, 0.08, 0.05, 1.0)
    ramp.color_ramp.elements[1].color = (0.20, 0.14, 0.09, 1.0)
    
    # Darker brown leather for cushions
    principled.inputs['Base Color'].default_value = (0.18, 0.12, 0.08, 1.0)
    principled.inputs['Roughness'].default_value = 0.25
    principled.inputs['IOR'].default_value = 1.4
    principled.inputs['Metallic'].default_value = 0.0
    
    # Connect nodes
    cushion_leather.node_tree.links.new(noise.outputs['Fac'], ramp.inputs['Fac'])
    cushion_leather.node_tree.links.new(ramp.outputs['Color'], principled.inputs['Base Color'])
    cushion_leather.node_tree.links.new(principled.outputs['BSDF'], output.inputs['Surface'])
...
if __name__ == "__main__":
    ...
    print("Luxury leather couch generation complete! Adjust camera angle as needed.")