...
def create_armrest(width, depth, height, location, name):
    ...
    bmesh.ops.bevel(bm, geom=edges_to_bevel, offset=0.1, segments=4, 
...
def create_seat_cushion(width, depth, height, location, name):
    ...
    # Bevel for cushion softness - increased for thicker look
    edges_to_bevel = [e for e in bm.edges]
...
def create_back_cushion(width, depth, height, location, name, angle=0):
    ...
    # Bevel for cushion softness - increased for thicker look
    edges_to_bevel = [e for e in bm.edges]
    bmesh.ops.bevel(bm, geom=edges_to_bevel, offset=0.07, segments=4, 
                    profile=0.7, affect='EDGES')
...
def create_couch_section(width, depth, base_height, back_height, location, section_name, num_cushions=3):
    ...
    armrest_width = 0.3
    armrest_height = 0.65
    seat_height = 0.18  # Increased from 0.15 for thicker cushions
    back_cushion_thickness = 0.22  # Increased from 0.20
...
def create_couch_materials():
    ...
    # Main leather material - rich brown color
    ...
    ramp.color_ramp.elements[0].color = (0.15, 0.08, 0.04, 1.0)  # Dark brown
    ramp.color_ramp.elements[1].color = (0.25, 0.15, 0.08, 1.0)  # Medium brown
    ...
    bsdf.inputs['Base Color'].default_value = (0.22, 0.13, 0.06, 1.0)
    ...
    bsdf.inputs['Base Color'].default_value = (0.28, 0.18, 0.10, 1.0)
...
def setup_scene():
    ...
    # Add key light - warmer color
    bpy.ops.object.light_add(type='AREA', location=(4, -4, 5))
    key_light = bpy.context.active_object
    key_light.data.energy = 350
    key_light.data.size = 3
    key_light.data.color = (1.0, 0.9, 0.8)  # Warm white
    
    # Add fill light - also warmer
    bpy.ops.object.light_add(type='AREA', location=(-3, -2, 3))
    fill_light = bpy.context.active_object
    fill_light.data.energy = 180
    fill_light.data.size = 2
    fill_light.data.color = (1.0, 0.85, 0.7)  # Warm fill
