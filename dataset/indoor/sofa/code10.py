...
def create_seat_cushion(width, depth, height, location, name):
    ...
    edges_to_bevel = [e for e in bm.edges]
    bmesh.ops.bevel(bm, geom=edges_to_bevel, offset=0.06, segments=4, 
...
def create_back_cushion(width, depth, height, location, name, angle=0):
    ...
    # Bevel for cushion softness
    edges_to_bevel = [e for e in bm.edges]
    bmesh.ops.bevel(bm, geom=edges_to_bevel, offset=0.05, segments=4, 
                    profile=0.7, affect='EDGES')
...
def create_couch_section(width, depth, base_height, back_height, location, section_name, num_cushions=3):
    ...
    armrest_width = 0.4  # Increased from 0.3 for wider armrests
    armrest_height = 0.65
    seat_height = 0.18  # Increased from 0.15 for deeper cushions
    back_cushion_thickness = 0.25  # Increased from 0.20 for thicker back cushions
...
def create_couch_materials():
    ...
    # Main leather material - rich burgundy color
    ...
    # Add ColorRamp for noise
    ramp = nodes.new(type='ShaderNodeValToRGB')
    ramp.location = (-100, 200)
    ramp.color_ramp.elements[0].position = 0.4
    ramp.color_ramp.elements[0].color = (0.25, 0.08, 0.12, 1.0)  # Dark burgundy
    ramp.color_ramp.elements[1].position = 0.6
    ramp.color_ramp.elements[1].color = (0.45, 0.15, 0.20, 1.0)  # Lighter burgundy
    
    # Mix node for combining colors
    ...
    principled.inputs['Base Color'].default_value = (0.4, 0.12, 0.18, 1.0)
    ...
    mix.inputs[7].default_value = (0.4, 0.12, 0.18, 1.0)
    links.new(mix.outputs[2], principled.inputs['Base Color'])
    links.new(principled.outputs['BSDF'], output.inputs['Surface'])
    
    # Cushion leather material - slightly darker burgundy
    ...
    # Slightly darker burgundy for cushions
    principled.inputs['Base Color'].default_value = (0.35, 0.10, 0.15, 1.0)
...
if __name__ == "__main__":
    ...
    print("Couch generation complete! Adjust camera angle as needed.")