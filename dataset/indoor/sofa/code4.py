...
def create_armrest(width, depth, height, location, name):
    """Create a padded armrest with increased thickness"""
    ...
    # Scale to armrest dimensions - slightly thicker
    bmesh.ops.scale(bm, vec=(width * 1.15, depth, height), verts=bm.verts)
    
    # Bevel edges for soft, cushioned look
    edges_to_bevel = [e for e in bm.edges]
    bmesh.ops.bevel(bm, geom=edges_to_bevel, offset=0.08, segments=4, 
...
def create_seat_cushion(width, depth, height, location, name):
    """Create a comfortable seat cushion with enhanced beveling"""
    mesh = bpy.data.meshes.new(name)
    obj = bpy.data.objects.new(name, mesh)
    bpy.context.collection.objects.link(obj)
    ...
    # Increased bevel for more pronounced cushion softness
    edges_to_bevel = [e for e in bm.edges]
...
def create_back_cushion(width, depth, height, location, name, angle=0):
    """Create a back cushion with optional angle and enhanced beveling"""
    mesh = bpy.data.meshes.new(name)
    obj = bpy.data.objects.new(name, mesh)
    bpy.context.collection.objects.link(obj)
    ...
    # Enhanced bevel for luxury cushion appearance
    edges_to_bevel = [e for e in bm.edges]
...
def create_couch_section(width, depth, base_height, back_height, location, section_name, num_cushions=3):
    ...
    armrest_width = 0.35  # Increased from 0.3
    armrest_height = 0.65
    seat_height = 0.15
    back_cushion_thickness = 0.20
...
def create_realistic_couch():
    """Generate a luxury leather couch"""
    
    # Main couch dimensions
    main_width = 5.0
    main_depth = 1.8
    base_height = 0.45
    back_height = 1.7
    ...
    # Create materials
    create_luxury_leather_materials()
    apply_materials()
    
    print("Luxury leather couch generated successfully!")

def create_luxury_leather_materials():
    ...
    # Add noise for leather texture variation
    ...
    # Color ramp for subtle leather variation
    ramp = nodes.new(type='ShaderNodeValToRGB')
    ramp.location = (-100, 100)
    ramp.color_ramp.elements[0].color = (0.12, 0.08, 0.05, 1.0)  # Dark brown
    ramp.color_ramp.elements[1].color = (0.18, 0.12, 0.08, 1.0)  # Lighter brown
    ...
    # Rich dark brown leather base color
    bsdf.inputs['Base Color'].default_value = (0.15, 0.10, 0.06, 1.0)
    bsdf.inputs['Roughness'].default_value = 0.4  # Leather roughness
    bsdf.inputs['IOR'].default_value = 1.5
    
    # Connect nodes
    links.new(noise.outputs['Fac'], ramp.inputs['Fac'])
    links.new(ramp.outputs['Color'], mix.inputs[6])
    mix.inputs[7].default_value = (0.18, 0.12, 0.08, 1.0)  # Base leather color
    ...
    links = cushion_leather.node_tree.links
    nodes.clear()
    ...
    bsdf.inputs['Base Color'].default_value = (0.18, 0.13, 0.09, 1.0)
...
def apply_materials():
    """Apply leather materials to couch components"""
