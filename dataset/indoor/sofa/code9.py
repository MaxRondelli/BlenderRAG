...
def create_seat_cushion(width, depth, height, location, name):
    """Create a comfortable seat cushion with increased thickness"""
    mesh = bpy.data.meshes.new(name)
    obj = bpy.data.objects.new(name, mesh)
    bpy.context.collection.objects.link(obj)
...
def create_couch_section(width, depth, base_height, back_height, location, section_name, num_cushions=3):
    ...
    armrest_width = 0.3
    armrest_height = 0.65
    seat_height = 0.2  # Increased from 0.15 for deeper cushions
...
def create_realistic_couch():
    """Generate a realistic leather couch"""
...
def create_couch_materials():
    ...
    # Add color ramp for leather variation
    ramp = nodes.new(type='ShaderNodeValToRGB')
    ramp.location = (0, 0)
    ramp.color_ramp.elements[0].color = (0.2, 0.12, 0.06, 1.0)
    ramp.color_ramp.elements[1].color = (0.3, 0.18, 0.1, 1.0)
    ...
    # Slightly lighter brown leather for cushions
    bsdf.inputs['Base Color'].default_value = (0.28, 0.18, 0.1, 1.0)
    ...
    ramp = nodes.new(type='ShaderNodeValToRGB')
    ramp.location = (0, 0)
    ramp.color_ramp.elements[0].color = (0.24, 0.15, 0.08, 1.0)
    ramp.color_ramp.elements[1].color = (0.32, 0.21, 0.12, 1.0)
...
def setup_scene():
    ...
    # Add rim light for leather highlights
