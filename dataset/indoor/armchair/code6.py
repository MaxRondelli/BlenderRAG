...
# Create materials
# Deep navy blue fabric
fabric_material = create_material("Fabric_Navy", (0.15, 0.22, 0.45, 1.0), roughness=0.8)
# Rose gold metal for legs
metal_material = create_material("Metal_RoseGold", (0.85, 0.65, 0.55, 1.0), roughness=0.15, metallic=0.95)

# ==================== SEAT BASE ====================
# Create the main seat cushion - slightly thicker and more rounded
...
# Add more pronounced subdivision for plush look
...
# ==================== BACKREST ====================
# Create curved backrest with increased height and plushness
bpy.ops.mesh.primitive_cube_add(size=1, location=(0, -0.7, 1.15))
backrest = bpy.context.active_object
backrest.name = "Backrest"
backrest.scale = (1.35, 0.25, 1.45)

# Rotate backrest to angle it back slightly more
backrest.rotation_euler = (math.radians(18), 0, 0)

# Apply transformations
bpy.ops.object.transform_apply(scale=True, rotation=True)

# Enter edit mode to add more subdivisions
bpy.ops.object.mode_set(mode='EDIT')
bpy.ops.mesh.subdivide(number_cuts=5)
bpy.ops.object.mode_set(mode='OBJECT')

# Add subdivision surface for extra smooth curves
...
# ==================== LEFT ARMREST ====================
# Create left armrest with increased thickness
bpy.ops.mesh.primitive_cube_add(size=1, location=(-0.8, -0.15, 0.8))
left_armrest = bpy.context.active_object
left_armrest.name = "Left_Armrest"
left_armrest.scale = (0.3, 1.05, 0.7)

# Rotate to angle inward slightly
left_armrest.rotation_euler = (math.radians(12), 0, math.radians(-6))

# Apply transformations
bpy.ops.object.transform_apply(scale=True, rotation=True)

# Add subdivisions for plush appearance
bpy.ops.object.mode_set(mode='EDIT')
...
# ==================== RIGHT ARMREST ====================
# Mirror for right armrest with increased thickness
bpy.ops.mesh.primitive_cube_add(size=1, location=(0.8, -0.15, 0.8))
right_armrest = bpy.context.active_object
right_armrest.name = "Right_Armrest"
right_armrest.scale = (0.3, 1.05, 0.7)

# Rotate to angle inward slightly (mirrored)
right_armrest.rotation_euler = (math.radians(12), 0, math.radians(6))
...
# ==================== LEGS ====================
# Create 4 angled rose gold legs - slightly thicker
# Front left leg
bpy.ops.mesh.primitive_cylinder_add(radius=0.035, depth=0.52, location=(-0.6, 0.5, 0.26))
...
empty.name = "Modern_Navy_Armchair"
