...
# Create materials
# Rich brown leather color
leather_material = create_material("Leather_Brown", (0.4, 0.25, 0.15, 1.0), roughness=0.3)
# Brass metal for legs
brass_material = create_material("Brass_Metal", (0.8, 0.65, 0.35, 1.0), roughness=0.1, metallic=0.9)

# ==================== SEAT BASE ====================
# Create the main seat cushion - slightly larger and thicker
bpy.ops.mesh.primitive_cube_add(size=1, location=(0, 0, 0.48))
seat = bpy.context.active_object
seat.name = "Seat_Cushion"
seat.scale = (2.0, 2.0, 0.6)
...
seat.data.materials.append(leather_material)

# ==================== BACKREST ====================
# Create curved backrest - slightly taller
...
# Rotate backrest to angle it back
...
# Add array modifier for tufted segments
bpy.ops.object.mode_set(mode='EDIT')
...
backrest.data.materials.append(leather_material)

# ==================== LEFT ARMREST ====================
# Create left armrest - slightly wider and more padded
bpy.ops.mesh.primitive_cube_add(size=1, location=(-0.85, -0.15, 0.78))
...
left_armrest.data.materials.append(leather_material)

# ==================== RIGHT ARMREST ====================
# Mirror for right armrest - slightly wider and more padded
bpy.ops.mesh.primitive_cube_add(size=1, location=(0.85, -0.15, 0.78))
...
right_armrest.data.materials.append(leather_material)

# ==================== LEGS ====================
# Create 4 angled brass legs - slightly thicker
# Front left leg
bpy.ops.mesh.primitive_cylinder_add(radius=0.035, depth=0.5, location=(-0.6, 0.5, 0.25))
front_left_leg = bpy.context.active_object
front_left_leg.name = "Front_Left_Leg"
front_left_leg.rotation_euler = (math.radians(-8), 0, math.radians(-12))
front_left_leg.data.materials.append(brass_material)

# Front right leg
bpy.ops.mesh.primitive_cylinder_add(radius=0.035, depth=0.5, location=(0.6, 0.5, 0.25))
front_right_leg = bpy.context.active_object
front_right_leg.name = "Front_Right_Leg"
front_right_leg.rotation_euler = (math.radians(-8), 0, math.radians(12))
front_right_leg.data.materials.append(brass_material)

# Back left leg
bpy.ops.mesh.primitive_cylinder_add(radius=0.035, depth=0.5, location=(-0.6, -0.5, 0.25))
back_left_leg = bpy.context.active_object
back_left_leg.name = "Back_Left_Leg"
back_left_leg.rotation_euler = (math.radians(8), 0, math.radians(-12))
back_left_leg.data.materials.append(brass_material)

# Back right leg
bpy.ops.mesh.primitive_cylinder_add(radius=0.035, depth=0.5, location=(0.6, -0.5, 0.25))
back_right_leg = bpy.context.active_object
back_right_leg.name = "Back_Right_Leg"
back_right_leg.rotation_euler = (math.radians(8), 0, math.radians(12))
back_right_leg.data.materials.append(brass_material)
