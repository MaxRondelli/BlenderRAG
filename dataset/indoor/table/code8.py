...
# ============================================
# MAHOGANY TABLE WITH THICKER LEGS - SEPARATE OBJECTS
...
def create_simple_realistic_table():
    ...
    # Dimensions (meters) - thicker elements
    table_width = 1.8
    table_depth = 0.9
    table_height = 0.5
    top_thickness = 0.08  # Increased from 0.05
    leg_size = 0.08  # Increased from 0.05
    ...
    for obj in all_parts:
        ...
        bevel.width = 0.008  # Slightly increased
        bevel.segments = 3
    ...
    # Mahogany color - darker reddish brown
    bsdf.inputs['Base Color'].default_value = (0.35, 0.15, 0.08, 1.0)
    bsdf.inputs['Roughness'].default_value = 0.2  # Slightly more glossy
    ...
    # Noise texture for mahogany grain pattern
    ...
    # Color ramp for mahogany tones
    ramp = nodes.new('ShaderNodeValToRGB')
    ramp.location = (-200, 0)
    ramp.color_ramp.elements[0].color = (0.25, 0.08, 0.04, 1.0)  # Dark mahogany
    ramp.color_ramp.elements[1].color = (0.45, 0.22, 0.12, 1.0)  # Light mahogany
    ...
    print(f"Created mahogany table with {len(all_parts)} separate objects")
    print(f"  - 1 table top at z={table_top_z}")
    print(f"  - 4 thicker legs from z=0 to z={leg_height}")
...
def main():
    """Main function."""
    print("=== Creating Mahogany Table with Thicker Elements (SEPARATE OBJECTS) ===")
    
    cleanup_scene()
    print("✓ Scene cleaned")
    
    table_parts = create_simple_realistic_table()
    print("✓ Mahogany table created with separate objects")
    print("  - Table top and legs are independent objects")
    print("  - Thicker legs and tabletop for robust appearance")
