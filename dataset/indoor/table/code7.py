...
# ============================================
# MODERN GLASS TABLE - SEPARATE OBJECTS
...
def create_modern_glass_table():
    ...
    leg_size = 0.04  # Slightly thinner chrome legs
    ...
    # Glass properties with blue tint
    ...
    # === CREATE CHROME MATERIAL FOR LEGS ===
    ...
    # Chrome properties
    bsdf.inputs['Base Color'].default_value = (0.9, 0.9, 0.9, 1.0)  # Light metallic
    ...
    # Chrome material to legs
    for obj in all_parts[1:]:  # Skip the first object (table top)
        if obj.data.materials:
            obj.data.materials[0] = chrome_mat
        else:
            obj.data.materials.append(chrome_mat)
    
    print(f"Created modern glass table with {len(all_parts)} separate objects")
...
def setup_lighting_and_camera():
    ...
    light3.data.size = 2
    light3.rotation_euler = (math.radians(45), 0, 0)
...
def main():
    ...
    print("  - Glass table top and chrome legs are independent objects")
    ...
    print("  - TableTop (glass material, separate object)")
    print("  - Leg1, Leg2, Leg3, Leg4 (chrome material, separate objects)")
