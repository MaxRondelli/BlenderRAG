...
# Wine glass dimensions (wider bowl, slightly different proportions)
bowl_bottom_radius = 0.018  # 1.8cm at bottom of bowl (wider than champagne flute)
bowl_top_radius = 0.038     # 3.8cm at rim (wider for wine glass)
bowl_height = 0.12          # 12cm bowl height (slightly shorter)
stem_radius = 0.003         # 3mm stem thickness (slightly thicker)
stem_height = 0.10          # 10cm stem height (shorter)
base_radius = 0.035         # 3.5cm base radius
base_thickness = 0.004      # 4mm base thickness (slightly thicker)
...
# Add subdivision surface modifier with edge split
...
# STEP 6: Create amber-tinted glass material
mat = bpy.data.materials.new(name="Amber_Glass_Material")
mat.use_nodes = True
nodes = mat.node_tree.nodes
nodes.clear()

# Glass BSDF with amber tint
...
glass_bsdf.inputs['Color'].default_value = (1.0, 0.8, 0.4, 1.0)  # Amber color
...
bpy.ops.object.light_add(type='AREA', location=(0, -0.4, 0.35))
...
print("Material: Amber-tinted glass with slightly higher IOR and minimal roughness")