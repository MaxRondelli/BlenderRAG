...
# Wine glass dimensions (wider bowl, shorter stem)
bowl_bottom_radius = 0.020      # 2.0cm at bottom of bowl (wider)
bowl_top_radius = 0.040         # 4.0cm at rim (much wider)
bowl_height = 0.12              # 12cm bowl height (shorter)
stem_radius = 0.003             # 3mm stem thickness (slightly thicker)
stem_height = 0.08              # 8cm stem height (much shorter)
base_radius = 0.035             # 3.5cm base radius (slightly larger)
base_thickness = 0.004          # 4mm base thickness (thicker)
glass_thickness = 0.002         # 2mm glass wall thickness (thicker)
...
stem_penetration = 0.003  # 3mm penetration into bowl
...
# STEP 6: Create emerald green glass material
mat = bpy.data.materials.new(name="Emerald_Glass_Material")
mat.use_nodes = True
...
glass_bsdf.inputs['Color'].default_value = (0.2, 0.8, 0.4, 1.0)  # Emerald green
...
# STEP 7: Setup camera
bpy.ops.object.camera_add(location=(0.20, -0.20, 0.15))
...
bpy.ops.object.light_add(type='AREA', location=(0, -0.25, 0.3))
...
print("Wine glass generated successfully!")
print(f"Dimensions:")
...
print(f"  - Stem diameter: {stem_radius * 2 * 1000:.1f}mm")
print(f"  - Base diameter: {base_radius * 2 * 100:.1f}cm")
print(f"  - Glass thickness: {glass_thickness * 1000:.1f}mm")
