...
MATTRESS_HEIGHT = 0.30  # 30cm thick mattress (thicker for luxury)
...
FRAME_THICKNESS = 0.08  # 8cm thick frame (thicker for luxury)
PILLOW_WIDTH = 0.50  # 50cm wide pillow
PILLOW_LENGTH = 0.50  # 70cm long pillow
PILLOW_HEIGHT = 0.15  # 15cm thick pillow (thicker)
...
def create_bed_frame():
    ...
    create_brass_material(bed_frame)
...
def create_headboard():
    ...
    # Apply burgundy velvet material
    create_velvet_material(headboard, color=(0.4, 0.1, 0.15, 1.0))  # Rich burgundy
...
def create_footboard():
    ...
    # Apply brass material
    create_brass_material(footboard)
...
def create_mattress():
    ...
    create_fabric_material(mattress, color=(0.95, 0.93, 0.88, 1.0))
...
def create_pillow(position, rotation=0):
    ...
    # Apply burgundy pillow material
    create_velvet_material(pillow, color=(0.45, 0.15, 0.20, 1.0), roughness=0.3)
    
    return pillow

def create_brass_material(obj):
    """
    Creates a brass material for the bed frame
    """
    mat = bpy.data.materials.new(name="BrassMaterial")
    mat.use_nodes = True
    ...
    # Brass color (golden-bronze)
    principled_node.inputs['Base Color'].default_value = (0.72, 0.55, 0.26, 1.0)
...
def create_velvet_material(obj, color=(0.4, 0.1, 0.15, 1.0), roughness=0.7):
    """
    Creates a velvet material for luxury upholstery
    """
    mat = bpy.data.materials.new(name="VelvetMaterial")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
...
if __name__ == "__main__":
    print("=" * 50)
    print("Creating Luxury Brass Bed")
    print("=" * 50)
    ...
    print("  - Brass bed frame with legs")
    print("  - Burgundy velvet headboard (padded)")
    print("  - Brass footboard")
    print("  - Premium thick mattress")
    print("  - 2 Burgundy velvet pillows")
    print("\nAll components are created with luxury materials and finishes.")
