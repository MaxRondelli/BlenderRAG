...
MATTRESS_HEIGHT = 0.20  # 20cm thick mattress (lower profile)
FRAME_HEIGHT = 0.25  # Lower frame base height from ground
HEADBOARD_HEIGHT = 0.90  # 90cm tall headboard (lower than original)
FOOTBOARD_HEIGHT = 0.25  # 25cm tall footboard (much lower)
FRAME_THICKNESS = 0.04  # 4cm thick frame (slightly thinner)
PILLOW_WIDTH = 0.50  # 50cm wide pillow
PILLOW_LENGTH = 0.50  # 70cm long pillow
PILLOW_HEIGHT = 0.08  # 8cm thick pillow (flatter)
...
def create_headboard():
    ...
    headboard.scale = (BED_WIDTH/2 + FRAME_THICKNESS, FRAME_THICKNESS/2, HEADBOARD_HEIGHT/2 + 0.25)
    bpy.ops.object.transform_apply(scale=True)
    ...
    # Apply cream fabric material
    create_fabric_material(headboard, color=(0.95, 0.92, 0.86, 1.0))  # Cream color
...
def create_mattress():
    ...
    create_fabric_material(mattress, color=(0.98, 0.96, 0.92, 1.0))
...
def create_pillow(position, rotation=0):
    ...
    create_fabric_material(pillow, color=(0.96, 0.93, 0.88, 1.0), roughness=0.7)
    
    return pillow

def create_wood_material(obj):
    """
    Creates a light oak wood material for the bed frame
    """
    mat = bpy.data.materials.new(name="LightOakMaterial")
    mat.use_nodes = True
    ...
    # Light oak color (warm blonde wood)
    principled_node.inputs['Base Color'].default_value = (0.82, 0.71, 0.55, 1.0)
...
def create_fabric_material(obj, color=(0.9, 0.9, 0.9, 1.0), roughness=0.8):
    ...
    """
    mat = bpy.data.materials.new(name="CreamFabricMaterial")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
...
def setup_scene():
    ...
    # Add rim light
    bpy.ops.object.light_add(type='AREA', location=(0, 4, 1.8))
...
if __name__ == "__main__":
    ...
    print("\nThe bed includes:")
    print("  - Low-profile platform frame with legs")
    print("  - Lower headboard (cream fabric)")
    print("  - Minimal footboard")
    print("  - Thinner mattress")
    print("  - 2 Flatter pillows")
    print("  - Light oak wood finish")
    print("\nScandinavian minimalist design with cream and light oak colors.")
