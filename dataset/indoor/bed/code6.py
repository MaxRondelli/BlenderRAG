...
FRAME_HEIGHT = 0.25  # Lower frame base height from ground
HEADBOARD_HEIGHT = 1.00  # 100cm tall headboard (lower)
FOOTBOARD_HEIGHT = 0.45  # 45cm tall footboard (lower)
FRAME_THICKNESS = 0.08  # 8cm thick frame (thicker for modern look)
...
def create_headboard():
    """
    Creates a tufted leather headboard
    """
    bpy.ops.mesh.primitive_cube_add(
    ...
    headboard.scale = (BED_WIDTH/2 + FRAME_THICKNESS, FRAME_THICKNESS/2, HEADBOARD_HEIGHT/2 + 0.3)
    bpy.ops.object.transform_apply(scale=True)
    ...
    # Add displacement for tufted effect
    displace_mod = headboard.modifiers.new(name="Displace", type='DISPLACE')
    displace_mod.strength = 0.05
    
    bpy.ops.object.shade_smooth()
    
    # Apply leather material
    create_leather_material(headboard, color=(0.15, 0.10, 0.08, 1.0))  # Dark brown leather
...
def create_footboard():
    ...
    """
    bpy.ops.mesh.primitive_cube_add(
        location=(0, BED_LENGTH/2 + FRAME_THICKNESS/2, FRAME_HEIGHT + FOOTBOARD_HEIGHT/2-0.25)
    )
...
def create_mattress():
    ...
    # Apply mattress material (white fabric)
    create_fabric_material(mattress, color=(0.95, 0.95, 0.95, 1.0))
...
def create_pillow(position, rotation=0):
    ...
    # Apply pillow material (off-white)
    create_fabric_material(pillow, color=(0.98, 0.97, 0.95, 1.0), roughness=0.6)
    
    return pillow

def create_wood_material(obj):
    """
    Creates a dark espresso wood material for the bed frame
    """
    mat = bpy.data.materials.new(name="EspressoWoodMaterial")
    mat.use_nodes = True
    ...
    # Espresso wood color (very dark brown)
    principled_node.inputs['Base Color'].default_value = (0.08, 0.06, 0.04, 1.0)
...
def create_leather_material(obj, color=(0.15, 0.10, 0.08, 1.0)):
    """
    Creates a leather material for the headboard
    """
    mat = bpy.data.materials.new(name="LeatherMaterial")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
    ...
    mix_node.inputs['A'].default_value = color
    mix_node.inputs['B'].default_value = (color[0] * 0.8, color[1] * 0.8, color[2] * 0.8, 1.0)
    ...
    links.new(ramp_node.outputs['Color'], mix_node.inputs['Factor'])
...
if __name__ == "__main__":
    print("=" * 50)
    print("Creating Modern Platform Bed")
    print("=" * 50)
    ...
    print("\n" + "=" * 50)
    print("Modern Platform Bed created successfully!")
    print("=" * 50)
    ...
    print("\nThe bed includes:")
    print("  - Low platform bed frame with legs")
    print("  - Tufted leather headboard")
    print("  - Lower footboard")
    print("  - Mattress")
    print("  - 2 Pillows")
    print("\nAll components feature modern styling with espresso wood finish.")
