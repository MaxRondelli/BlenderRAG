...
MATTRESS_HEIGHT = 0.22  # 22cm thick mattress (slightly thinner)
FRAME_HEIGHT = 0.25  # Lower frame base height from ground
HEADBOARD_HEIGHT = 0.85  # Lower 85cm tall headboard
FOOTBOARD_HEIGHT = 0.45  # Lower 45cm tall footboard
FRAME_THICKNESS = 0.08  # Thicker 8cm frame for modern look
PILLOW_WIDTH = 0.50  # 50cm wide pillow
PILLOW_LENGTH = 0.50  # 50cm long pillow
...
def create_headboard():
    ...
    # Apply cream upholstery material
    create_fabric_material(headboard, color=(0.9, 0.85, 0.75, 1.0))  # Cream color
...
def create_pillow(position, rotation=0):
    ...
    create_fabric_material(pillow, color=(0.95, 0.92, 0.85, 1.0), roughness=0.6)
    
    return pillow

def create_wood_material(obj):
    """
    Creates a light wood material for the bed frame
    """
    mat = bpy.data.materials.new(name="LightWoodMaterial")
    mat.use_nodes = True
    ...
    # Light wood color (maple/birch)
    principled_node.inputs['Base Color'].default_value = (0.85, 0.72, 0.55, 1.0)
...
if __name__ == "__main__":
    ...
    print("\nThe bed includes:")
    print("  - Low-profile bed frame with legs")
    print("  - Lower headboard (padded)")
    print("  - Lower footboard")
    print("  - Mattress")
    print("  - 2 Pillows")
    print("\nAll components feature modern proportions and light finishes.")
