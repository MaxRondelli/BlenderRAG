...
FRAME_THICKNESS = 0.08  # 8cm thick frame (increased from 5cm)
...
def create_bed_frame():
    ...
    # Create legs (4 corners) - thicker legs
    leg_positions = [
...
def create_headboard():
    ...
    create_fabric_material(headboard, color=(0.95, 0.92, 0.88, 1.0))  # Cream color
...
def create_mattress():
    ...
    # Apply mattress material (cream white fabric)
    create_fabric_material(mattress, color=(0.98, 0.96, 0.93, 1.0))
...
def create_pillow(position, rotation=0):
    ...
    # Apply pillow material (warm cream)
    create_fabric_material(pillow, color=(0.97, 0.94, 0.90, 1.0), roughness=0.6)
...
def create_wood_material(obj):
    ...
    principled_node.inputs['Base Color'].default_value = (0.65, 0.45, 0.25, 1.0)
