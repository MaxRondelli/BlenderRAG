...
def create_glass_door():
    ...
    # Create modern vertical door handle
    handle_height = 1.05  # Center height of handle - MODIFY THIS to move handle up/down (Z-axis)
    handle_offset_x = door_width/2 + -0.3  # Distance from edge of door (X-axis position)
    handle_depth = 0.035  # How far handle extends from door
    handle_length = 0.30  # Vertical length of handle bar
    handle_y_offset = -0.021  # ADD THIS VALUE to all Y coordinates to move handle forward/backward
    
    # Handle mounting plate (attaches to door)
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.018,
        depth=0.006,
        location=(handle_offset_x, door_thickness/2 + 0.003 + handle_y_offset, handle_height + handle_length/2 - 0.015)
    )
    ...
    # Connecting rods (from door to handle bar)
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.010,
        depth=handle_depth,
        location=(handle_offset_x, door_thickness/2 + handle_depth/2 + handle_y_offset, handle_height + handle_length/2 - 0.015)
    )
...
def create_glass_material():
    ...
    # Glass properties
    node_bsdf.inputs['Base Color'].default_value = (0.9, 0.9, 0.95, 1.0)
    node_bsdf.inputs['Alpha'].default_value = 0.1
