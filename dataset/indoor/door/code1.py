...
def create_wooden_door():
    ...
    # Door dimensions (in meters) - slightly thicker for rustic appearance
    door_width = 0.9
    door_height = 2.1
    ...
    # Panel recess parameters - deeper for rustic look
    panel_inset = 0.08
    panel_depth = 0.018  # Increased depth for more pronounced panels
    ...
    # Add bevels to edges for realism - slightly larger for rustic look
    ...
    handle_base.rotation_euler = (1.5708, 0.5, 0)
    ...
    # Apply bronze material to handle parts
    if len(handle_base.data.materials) == 0:
        handle_base.data.materials.append(bpy.data.materials.get("Bronze_Material"))
    else:
        handle_base.data.materials[0] = bpy.data.materials.get("Bronze_Material")
    
    if len(handle_lever.data.materials) == 0:
        handle_lever.data.materials.append(bpy.data.materials.get("Bronze_Material"))
    else:
        handle_lever.data.materials[0] = bpy.data.materials.get("Bronze_Material")
...
def create_wood_material():
    """Create a realistic dark walnut wood material"""
    ...
    # Dark walnut color
    node_bsdf.inputs['Base Color'].default_value = (0.22, 0.12, 0.06, 1.0)
    node_bsdf.inputs['Roughness'].default_value = 0.6  # More matte for rustic look
    ...
    node_mapping.inputs['Scale'].default_value = (3, 3, 3)  # Larger grain pattern
    ...
    node_wave.inputs['Distortion'].default_value = 3.0  # More distortion for rustic look
    ...
    # Darker walnut tones
    node_color_ramp.color_ramp.elements[0].color = (0.15, 0.08, 0.04, 1)
    node_color_ramp.color_ramp.elements[1].color = (0.3, 0.16, 0.08, 1)
    ...
    node_bump.inputs['Strength'].default_value = 0.15  # More pronounced texture
...
def create_bronze_material():
    """Create a bronze material for the door handle"""
    ...
    # Bronze color and properties
    node_bsdf.inputs['Base Color'].default_value = (0.7, 0.45, 0.25, 1.0)  # Bronze color
    node_bsdf.inputs['Metallic'].default_value = 1.0
    node_bsdf.inputs['Roughness'].default_value = 0.4  # Slightly rougher than polished metal
