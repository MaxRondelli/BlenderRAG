...
def create_wooden_door():
    ...
    # Door dimensions (in meters) - slightly thicker variation
    door_width = 0.9
    door_height = 2.1
    door_thickness = 0.055  # Increased from 0.045
    ...
    panel_depth = 0.015  # Slightly deeper panels from 0.012
...
def create_wood_material():
    """Create a realistic dark mahogany wood material"""
    ...
    # Dark mahogany color (darker and richer than original)
    node_bsdf.inputs['Base Color'].default_value = (0.22, 0.08, 0.04, 1.0)
    node_bsdf.inputs['Roughness'].default_value = 0.35  # Slightly smoother
    ...
    node_wave.inputs['Detail'].default_value = 2.0
    
    node_color_ramp = nodes.new(type='ShaderNodeValToRGB')
    node_color_ramp.location = (-200, 100)
    # Darker mahogany tones
    node_color_ramp.color_ramp.elements[0].color = (0.15, 0.05, 0.02, 1)
    node_color_ramp.color_ramp.elements[1].color = (0.30, 0.12, 0.06, 1)
...
def create_metal_material():
    """Create a brass metal material for the door handle"""
    ...
    # Brass color appearance (golden yellow tone)
    node_bsdf.inputs['Base Color'].default_value = (0.9, 0.7, 0.3, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 1.0
    node_bsdf.inputs['Roughness'].default_value = 0.25  # Slightly more polished
