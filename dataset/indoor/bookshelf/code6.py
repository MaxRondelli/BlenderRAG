...
def create_bookshelf(location, num_shelves=5):
    ...
    # Darker wood color
    wood_color = (0.2, 0.15, 0.1)
    wood_mat = create_material("WoodMat", wood_color, roughness=0.6)
    
    # Metal accent material
    metal_color = (0.4, 0.4, 0.4)
    metal_mat = create_material("MetalMat", metal_color, metallic=0.8, roughness=0.2)
    
    # Create vertical sides with metal accents
    for x_offset in [-shelf_width/2, shelf_width/2]:
        ...
        # Metal accent strip
        bpy.ops.mesh.primitive_cube_add(
            location=(location[0] + x_offset * 0.95, location[1] + shelf_depth/3, location[2] + shelf_height/2),
            scale=(0.01, 0.01, shelf_height/2)
        )
        accent = bpy.context.active_object
        accent.data.materials.append(metal_mat)
    ...
    for i in range(num_shelves + 1):
        ...
        if i < num_shelves:
            ...
            for j in range(num_books):
                book_colors = [
                    (0.7, 0.1, 0.1), (0.1, 0.3, 0.7), (0.2, 0.5, 0.2),
                    (0.5, 0.2, 0.5), (0.7, 0.5, 0.1), (0.4, 0.2, 0.1),
                    (0.1, 0.4, 0.4), (0.6, 0.3, 0.2), (0.3, 0.3, 0.3),
                    (0.8, 0.4, 0.0)
                ]
                
                book_x = location[0] - shelf_width/2 + 0.2 + (j * (shelf_width - 0.4) / num_books)
                
                # More varied book heights
                book_height = random.uniform(0.1, 0.28)
                book_thickness = random.uniform(0.015, 0.05)
                book_width = random.uniform(0.07, 0.13)
                ...
                rotation = (0, 0, random.uniform(-0.15, 0.15))
