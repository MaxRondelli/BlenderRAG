...
def create_book(location, rotation, scale, color):
    ...
    mat = create_material(f"BookMat_{random.randint(0, 10000)}", color, roughness=0.3)
...
def create_bookshelf(location, num_shelves=5):
    ...
    wood_mat = create_material("WoodMat", wood_color, roughness=0.8)
    
    # Brass accent material
    brass_color = (0.6, 0.4, 0.1)
    ...
    # Create shelves with brass edge trim
    for i in range(num_shelves + 1):
        ...
        # Brass front edge trim
        bpy.ops.mesh.primitive_cube_add(
            location=(location[0], location[1] + shelf_depth/2 - 0.01, shelf_z + 0.04),
            scale=(shelf_width/2 - 0.05, 0.005, 0.01)
        )
        trim = bpy.context.active_object
        trim.data.materials.append(brass_mat)
        
        # Add books on each shelf (except top)
        if i < num_shelves:
            num_books = random.randint(8, 15)
            for j in range(num_books):
                # Vintage leather book colors
                book_colors = [
                    (0.4, 0.15, 0.1), (0.2, 0.25, 0.15), (0.35, 0.2, 0.05),
                    (0.3, 0.1, 0.05), (0.45, 0.3, 0.15), (0.25, 0.2, 0.1),
                    (0.5, 0.25, 0.1), (0.3, 0.25, 0.2)
                ]
