...
def create_bookshelf(location, num_shelves=6):
    ...
    wood_mat = create_material("WalnutMat", wood_color)
    
    # Create vertical sides - slightly thicker
    ...
    # Create shelves - slightly thicker
    for i in range(num_shelves + 1):
        ...
        if i < num_shelves:
            num_books = random.randint(10, 18)
            for j in range(num_books):
                # More sophisticated, muted color palette
                book_colors = [
                    (0.4, 0.2, 0.15), (0.15, 0.25, 0.4), (0.2, 0.35, 0.2),
                    (0.35, 0.15, 0.25), (0.45, 0.35, 0.15), (0.3, 0.2, 0.1),
                    (0.15, 0.3, 0.3), (0.4, 0.25, 0.18), (0.25, 0.25, 0.25),
                    (0.3, 0.1, 0.1), (0.1, 0.2, 0.35), (0.2, 0.25, 0.15)
                ]
                
                book_x = location[0] - shelf_width/2 + 0.2 + (j * (shelf_width - 0.4) / num_books)
                
                book_height = random.uniform(0.15, 0.25)
                book_thickness = random.uniform(0.018, 0.045)
...
def main():
    ...
    bpy.ops.object.camera_add(location=(4.5, -3.5, 2.5))
