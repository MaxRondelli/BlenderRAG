...
def create_pot(location=(0, 0, 0), radius=0.32, height=0.38):
    ...
    # Add Subdivision Surface for smooth, professional appearance
    ...
    # Create pot material - COPPER COLOR
    pot_mat = create_material("PotMaterial", (0.85, 0.45, 0.25), metallic=0.95, roughness=0.15)
    ...
    rim_mat = create_material("RimMaterial", (0.82, 0.42, 0.22), metallic=0.92, roughness=0.18)
    rim.data.materials.append(rim_mat)
    
    # Create pot handles (elegant loop handles)
    ...
    for i, (hx, hy, hz) in enumerate(handle_positions):
        ...
        # Handle material - COPPER COLOR
        handle_mat = create_material(f"PotHandleMat_{i}", (0.88, 0.48, 0.28), metallic=0.90, roughness=0.20)
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(
        ...
        scale=(radius + 0.015, radius + 0.015, 0.065),
        segments=64,
        ring_count=32
    )
    ...
    lid_mat = create_material("LidMaterial", (0.83, 0.43, 0.23), metallic=0.92, roughness=0.17)
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        location=(location[0], location[1], lid_z + 0.075),
    ...
    # Dark wood knob material
    knob_mat = create_material("KnobMaterial", (0.15, 0.08, 0.04), metallic=0.02, roughness=0.85)
    knob_base.data.materials.append(knob_mat)
    
    # Knob top (rounded)
    bpy.ops.mesh.primitive_uv_sphere_add(
        location=(location[0], location[1], lid_z + 0.108),
...
def create_cookware_scene():
    ...
    # Create cooking pot with copper materials and thicker walls
    pot = create_pot(
        location=(0, 0, 0.01),
        radius=0.32,
        height=0.38
    ...
    print("Copper pot generated successfully!")
    print("Features: Copper materials, thicker walls, more pronounced dome lid")
