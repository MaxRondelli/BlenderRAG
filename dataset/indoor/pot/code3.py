...
def create_pot(location=(0, 0, 0), radius=0.36, height=0.35):
    """
    Create a realistic ceramic cooking pot with lid and handles
    
    Parameters:
    - location: (x, y, z) position
    - radius: radius of the pot (wider than original)
    - height: height of the pot (shorter than original)
    """
    ...
    # Inset the top face for wall thickness
    bpy.ops.mesh.inset(thickness=0.022, depth=0)
    
    # Extrude down to create hollow interior - REDUCED to keep flat bottom
    ...
    # Add Subdivision Surface for smooth, professional appearance - REDUCED levels
    ...
    # Create pot material (ceramic with warm earth tone glaze)
    pot_mat = create_material("PotMaterial", (0.68, 0.52, 0.38), metallic=0.05, roughness=0.25)
    pot_body.data.materials.append(pot_mat)
    
    # Create pot rim (rolled edge)
    bpy.ops.mesh.primitive_torus_add(
        location=(location[0], location[1], location[2] + height + 0.005),
        major_radius=radius - 0.01,
        minor_radius=0.014,
        major_segments=64,
        minor_segments=16
    )
    ...
    rim_mat = create_material("RimMaterial", (0.62, 0.48, 0.35), metallic=0.08, roughness=0.30)
    rim.data.materials.append(rim_mat)
    
    # Create pot handles (elegant loop handles) - FIXED POSITIONING
    handle_height = location[2] + height * 0.7
    handle_offset = radius + 0.07  # Distance from pot center
    ...
    for i, (hx, hy, hz) in enumerate(handle_positions):
        # Create torus for main handle loop
        bpy.ops.mesh.primitive_torus_add(
            location=(hx, hy, hz),
        ...
        # Handle material (brushed bronze/brass)
        handle_mat = create_material(f"PotHandleMat_{i}", (0.55, 0.42, 0.25), metallic=0.75, roughness=0.35)
        handle.data.materials.append(handle_mat)
        
        # Handle mounting bracket - POSITIONED ON THE POT SURFACE
        # Position mounts exactly at the pot radius
        mount_x = location[0] + (radius * 0.98 if i == 0 else -(radius * 0.98))
        
        # Upper mount
        bpy.ops.mesh.primitive_cylinder_add(
            location=(mount_x, hy, hz + 0.038),
            rotation=(0, math.radians(90), 0),
        ...
        connector_x = location[0] + (radius * 0.99 if i == 0 else -(radius * 0.99)) + (0.018 if i == 0 else -0.018)
        
        bpy.ops.mesh.primitive_cube_add(
            location=(connector_x, hy, hz),
            scale=(0.022, 0.016, 0.058)
        )
        ...
        connector_bevel.width = 0.004
        connector_bevel.segments = 2
        
        connector.data.materials.append(handle_mat)
    
    # Create realistic pot lid
    lid_z = location[2] + height + 0.028
    
    # Lid main body (slightly domed)
    bpy.ops.mesh.primitive_uv_sphere_add(
        location=(location[0], location[1], lid_z + 0.028),
        scale=(radius + 0.018, radius + 0.018, 0.045),
        segments=64,
        ring_count=32
    )
    ...
    lid_mat = create_material("LidMaterial", (0.66, 0.50, 0.36), metallic=0.06, roughness=0.28)
    lid.data.materials.append(lid_mat)
    
    # Lid rim (sits inside pot)
    bpy.ops.mesh.primitive_cylinder_add(
        location=(location[0], location[1], lid_z - 0.008),
        scale=(radius - 0.028, radius - 0.028, 0.018),
        vertices=64
    )
    ...
    lid_rim.data.materials.append(lid_mat)
    
    # Lid knob/handle (wooden design)
    bpy.ops.mesh.primitive_cylinder_add(
        location=(location[0], location[1], lid_z + 0.058),
    ...
    # Wooden knob material
    knob_mat = create_material("KnobMaterial", (0.45, 0.28, 0.15), metallic=0.02, roughness=0.65)
    knob_base.data.materials.append(knob_mat)
    
    # Knob top (rounded wooden cap)
    bpy.ops.mesh.primitive_uv_sphere_add(
        location=(location[0], location[1], lid_z + 0.092),
        scale=(0.032, 0.032, 0.022),
        segments=24,
        ring_count=16
    )
    ...
    print(f"Realistic ceramic pot created at {location}")
    return pot_body
...
def setup_scene():
    ...
    area.name = "FillLight"
    area.data.energy = 140
    area.data.size = 2.5
    
    # Add rim light for ceramic highlights
    bpy.ops.object.light_add(type='POINT', location=(1.5, 2, 1.8))
    point = bpy.context.active_object
    point.name = "RimLight"
    point.data.energy = 180
    
    print("Scene setup complete!")

def create_cookware_scene():
    """Generate a complete scene with ceramic pot"""
   
    # Create ceramic cooking pot with wider, shorter proportions
    pot = create_pot(
        location=(0, 0, 0.01),
        radius=0.36,
        height=0.35
    ...
    print("Realistic ceramic pot generated successfully!")
    print("Features: Ceramic glazed finish, wooden knob, bronze handles, wider proportions")
