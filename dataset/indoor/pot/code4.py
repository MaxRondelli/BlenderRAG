...
def create_pot(location=(0, 0, 0), radius=0.32, height=0.38):
    ...
    # Inset the top face for wall thickness - INCREASED for thicker walls
    ...
    # Create copper pot material - CHANGED to copper color
    pot_mat = create_material("PotMaterial", (0.84, 0.52, 0.35), metallic=0.95, roughness=0.25)
    pot_body.data.materials.append(pot_mat)
    
    # Create pot rim (rolled edge) - copper color
    ...
    rim_mat = create_material("RimMaterial", (0.82, 0.50, 0.33), metallic=0.93, roughness=0.28)
    ...
    for i, (hx, hy, hz) in enumerate(handle_positions):
        ...
        handle_mat = create_material(f"PotHandleMat_{i}", (0.80, 0.48, 0.31), metallic=0.90, roughness=0.30)
    ...
    # Lid main body (slightly domed) - copper
    ...
    lid_mat = create_material("LidMaterial", (0.82, 0.50, 0.33), metallic=0.93, roughness=0.26)
    lid.data.materials.append(lid_mat)
    
    # Lid rim (sits inside pot) - copper
    bpy.ops.mesh.primitive_cylinder_add(
    ...
    # Lid knob/handle (brass design) - CHANGED to brass
    bpy.ops.mesh.primitive_cylinder_add(
    ...
    # Brass knob material - CHANGED from black plastic to brass
    knob_mat = create_material("KnobMaterial", (0.85, 0.68, 0.35), metallic=0.88, roughness=0.18)
    knob_base.data.materials.append(knob_mat)
    
    # Knob top (rounded) - brass
    bpy.ops.mesh.primitive_uv_sphere_add(
...
def create_cookware_scene():
    ...
    print("Features: FLAT BASE, properly attached handles, domed lid, copper material with brass knob")
