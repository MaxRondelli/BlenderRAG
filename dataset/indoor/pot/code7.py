...
def create_pot(location=(0, 0, 0), radius=0.32, height=0.38):
    ...
    # Inset the top face for wall thickness - INCREASED THICKNESS
    bpy.ops.mesh.inset(thickness=0.028, depth=0)
    ...
    pot_mat = create_material("PotMaterial", (0.85, 0.52, 0.38), metallic=0.88, roughness=0.22)
    ...
    rim_mat = create_material("RimMaterial", (0.82, 0.49, 0.35), metallic=0.85, roughness=0.25)
    ...
    for i, (hx, hy, hz) in enumerate(handle_positions):
        ...
        # Handle material (copper finish)
        handle_mat = create_material(f"PotHandleMat_{i}", (0.83, 0.50, 0.36), metallic=0.85, roughness=0.26)
    ...
    # Create realistic pot lid - MORE DOMED
    lid_z = location[2] + height + 0.025
    
    # Lid main body (more pronounced dome)
    bpy.ops.mesh.primitive_uv_sphere_add(
        location=(location[0], location[1], lid_z + 0.035),
        scale=(radius + 0.015, radius + 0.015, 0.055),
        segments=64,
        ring_count=32
    )
    ...
    lid_mat = create_material("LidMaterial", (0.84, 0.51, 0.37), metallic=0.86, roughness=0.23)
    ...
    # Lid knob/handle (more elegant design)
    bpy.ops.mesh.primitive_cylinder_add(
        location=(location[0], location[1], lid_z + 0.065),
    ...
    # Black phenolic knob material
    knob_mat = create_material("KnobMaterial", (0.08, 0.08, 0.08), metallic=0.05, roughness=0.75)
    knob_base.data.materials.append(knob_mat)
    
    # Knob top (rounded)
    bpy.ops.mesh.primitive_uv_sphere_add(
        location=(location[0], location[1], lid_z + 0.098),
