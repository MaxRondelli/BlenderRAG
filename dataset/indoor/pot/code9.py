...
def create_pot(location=(0, 0, 0), radius=0.34, height=0.40):
    """
    ...
    # Inset the top face for wall thickness - thicker walls
    ...
    # Create pot material (copper finish)
    pot_mat = create_material("PotMaterial", (0.85, 0.42, 0.22), metallic=0.96, roughness=0.15)
    ...
    rim_mat = create_material("RimMaterial", (0.82, 0.40, 0.20), metallic=0.94, roughness=0.18)
    ...
    handle_offset = radius + 0.065  # Distance from pot center
    ...
    for i, (hx, hy, hz) in enumerate(handle_positions):
        # Create torus for main handle loop - thicker handles
        bpy.ops.mesh.primitive_torus_add(
            location=(hx, hy, hz),
        ...
        # Handle material (brass/copper alloy)
        handle_mat = create_material(f"PotHandleMat_{i}", (0.78, 0.55, 0.28), metallic=0.92, roughness=0.20)
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(
        ...
        scale=(radius + 0.018, radius + 0.018, 0.044),
        segments=64,
        ring_count=32
    )
    ...
    lid_mat = create_material("LidMaterial", (0.83, 0.41, 0.21), metallic=0.94, roughness=0.16)
    lid.data.materials.append(lid_mat)
    
    # Lid rim (sits inside pot) - thicker
    bpy.ops.mesh.primitive_cylinder_add(
    ...
    # Lid knob/handle (more elegant design) - brass knob
    bpy.ops.mesh.primitive_cylinder_add(
        location=(location[0], location[1], lid_z + 0.060),
    ...
    # Brass knob material
    knob_mat = create_material("KnobMaterial", (0.75, 0.52, 0.25), metallic=0.88, roughness=0.25)
    knob_base.data.materials.append(knob_mat)
    
    # Knob top (rounded)
    bpy.ops.mesh.primitive_uv_sphere_add(
        location=(location[0], location[1], lid_z + 0.095),
        scale=(0.032, 0.032, 0.020),
        segments=24,
        ring_count=16
    )
...
def create_cookware_scene():
    ...
    print("Realistic copper pot generated successfully!")
    print("Features: FLAT BASE, properly attached handles, domed lid, copper material")
