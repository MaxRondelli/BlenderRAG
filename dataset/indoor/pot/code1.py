...
def create_wood_material(name):
    """Create a wood material with procedural texture"""
    mat = bpy.data.materials.new(name=name)
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    nodes.clear()
    ...
    # Create noise texture for wood grain
    node_noise = nodes.new(type='ShaderNodeTexNoise')
    node_noise.location = (-200, 0)
    node_noise.inputs['Scale'].default_value = 15.0
    node_noise.inputs['Detail'].default_value = 8.0
    
    # Create ColorRamp for wood colors
    node_ramp = nodes.new(type='ShaderNodeValToRGB')
    node_ramp.location = (100, 0)
    node_ramp.color_ramp.elements[0].color = (0.3, 0.15, 0.08, 1)
    node_ramp.color_ramp.elements[1].color = (0.6, 0.35, 0.2, 1)
    ...
    mat.node_tree.links.new(node_ramp.outputs['Color'], node_bsdf.inputs['Base Color'])
    mat.node_tree.links.new(node_bsdf.outputs['BSDF'], node_output.inputs['Surface'])
...
def create_pot(location=(0, 0, 0), radius=0.32, height=0.38):
    ...
    pot_mat = create_material("PotMaterial", (0.72, 0.45, 0.20), metallic=0.95, roughness=0.15)
    ...
    rim_mat = create_material("RimMaterial", (0.70, 0.42, 0.18), metallic=0.92, roughness=0.18)
    ...
    for i, (hx, hy, hz) in enumerate(handle_positions):
        ...
        handle_mat = create_material(f"PotHandleMat_{i}", (0.68, 0.40, 0.16), metallic=0.90, roughness=0.20)
    ...
    lid_mat = create_material("LidMaterial", (0.69, 0.43, 0.19), metallic=0.93, roughness=0.17)
    ...
    # Wood knob material
    knob_mat = create_wood_material("KnobMaterial")
...
def create_cookware_scene():
    ...
    print("Features: FLAT BASE, properly attached handles, domed lid, copper material with wood knob")
