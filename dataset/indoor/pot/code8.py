...
def create_pot(location=(0, 0, 0), radius=0.32, height=0.38):
    """
    Create a rustic copper pot with lid and handles
    ...
    # Create copper pot material
    pot_mat = create_material("PotMaterial", (0.85, 0.45, 0.25), metallic=0.88, roughness=0.25)
    ...
    rim_mat = create_material("RimMaterial", (0.82, 0.42, 0.22), metallic=0.86, roughness=0.28)
    ...
    for i, (hx, hy, hz) in enumerate(handle_positions):
        ...
        # Copper handle material
        handle_mat = create_material(f"PotHandleMat_{i}", (0.88, 0.48, 0.28), metallic=0.85, roughness=0.30)
    ...
    lid_mat = create_material("LidMaterial", (0.84, 0.44, 0.24), metallic=0.87, roughness=0.26)
    ...
    # Traditional rounded knob - LARGER AND MORE ROUNDED
    bpy.ops.mesh.primitive_uv_sphere_add(
        location=(location[0], location[1], lid_z + 0.045),
        scale=(0.035, 0.035, 0.032),
        segments=32,
        ring_count=24
    )
    ...
    knob_subsurf = knob.modifiers.new(name="Subdivision", type='SUBSURF')
    knob_subsurf.levels = 3
    
    # Brass knob material
    knob_mat = create_material("KnobMaterial", (0.72, 0.55, 0.18), metallic=0.82, roughness=0.35)
    knob.data.materials.append(knob_mat)
    
    print(f"Rustic copper pot created at {location}")
    return pot_body
...
def create_cookware_scene():
    ...
    print("Rustic copper pot generated successfully!")
    print("Features: Copper coloring, thicker walls, rounded brass knob")
