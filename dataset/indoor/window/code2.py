...
def create_wood_material(name, base_color, roughness=0.6):
    ...
    color_ramp.color_ramp.elements[0].color = (base_color[0] * 0.5, base_color[1] * 0.5, base_color[2] * 0.4, 1)
...
def create_wooden_window():
    """Create a realistic wooden window with proper structure"""
    ...
    MUNTIN_THICKNESS = 0.08  # Thicker muntins for rustic appearance
    ...
    # Create weathered oak wood material
    wood_mat = create_wood_material(
        "WeatheredOakMaterial",
        base_color=(0.25, 0.18, 0.10, 1.0),  # More golden oak color
        roughness=0.8  # Higher roughness for weathered look
    )
    ...
    # ============ TINTED BLUE GLASS MATERIAL ============
    glass_mat = bpy.data.materials.new(name="TintedBlueGlass")
    ...
    glass_node.inputs['Color'].default_value = (0.85, 0.9, 1.0, 1.0)  # Light blue tint
    ...
    for obj in all_wood_objects:
        ...
        # Larger bevel for more rustic rounded edges
    ...
    window.name = "WeatheredOakWindow"
    ...
    print("\n" + "="*60)
    print("WEATHERED OAK WINDOW CREATED SUCCESSFULLY!")
    print("="*60)
