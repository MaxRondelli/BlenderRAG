...
def create_wood_material(name, base_color, roughness=0.8):
    """Create a procedural weathered wood material"""
...
def create_wooden_window():
    """Create a rustic wooden window with weathered finish"""
    
    # ============ DIMENSIONS (in meters) ============
    TOTAL_WIDTH = 1.2        # Total outer width
    TOTAL_HEIGHT = 1.2       # Total outer height
    FRAME_DEPTH = 0.08       # Depth (Z direction)
    FRAME_THICKNESS = 0.14   # Thicker frame for rustic look
    MUNTIN_THICKNESS = 0.07  # Thicker muntins
    GLASS_THICKNESS = 0.004  # Glass thickness
    GLASS_RECESS = 0.025     # Slightly deeper recess
    
    # Calculate inner dimensions
    INNER_WIDTH = TOTAL_WIDTH - (2 * FRAME_THICKNESS)
    INNER_HEIGHT = TOTAL_HEIGHT - (2 * FRAME_THICKNESS)
    
    # Create weathered wood material with darker color
    wood_mat = create_wood_material(
        "WeatheredWoodMaterial",
        base_color=(0.12, 0.08, 0.04, 1.0),  # Darker weathered brown
        roughness=0.8  # More rough for weathered look
    )
    ...
    glass_node.inputs['Color'].default_value = (0.85, 0.88, 0.75, 1.0)  # Slightly amber tint
    ...
    for obj in all_wood_objects:
        ...
        # Larger bevel for more weathered edges
