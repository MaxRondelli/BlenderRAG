...
def create_wood_material(name, base_color, roughness=0.6):
    ...
    wood_tex.inputs['Scale'].default_value = 3.0
    wood_tex.inputs['Distortion'].default_value = 2.0
    wood_tex.inputs['Detail'].default_value = 2.0
    
    color_ramp = nodes.new(type='ShaderNodeValToRGB')
    color_ramp.location = (-200, 100)
    color_ramp.color_ramp.elements[0].color = (base_color[0] * 0.6, base_color[1] * 0.6, base_color[2] * 0.6, 1)
...
def create_wooden_window():
    ...
    FRAME_THICKNESS = 0.14   # Increased thickness for rustic look
    MUNTIN_THICKNESS = 0.06  # Slightly thicker muntins
    ...
    # Create weathered oak wood material with darker tones
    wood_mat = create_wood_material(
        "WeatheredOakMaterial",
        base_color=(0.12, 0.08, 0.06, 1.0),  # Darker brown
        roughness=0.8  # More weathered surface
    )
    ...
    # ============ GLASS MATERIAL ============
    glass_mat = bpy.data.materials.new(name="BlueTintedGlass")
    ...
    glass_node.inputs['Color'].default_value = (0.85, 0.9, 1.0, 1.0)  # Blue tint
    ...
    print(f"Glass Panes: 4 (2×2 grid) with blue tint")
    print("="*60 + "\n")
