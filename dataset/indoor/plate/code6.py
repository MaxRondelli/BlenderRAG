...
PLATE_DEPTH = 0.08 # Deeper well (8cm) for bowl-like appearance
PLATE_THICKNESS = 0.005  # Thicker ceramic (5mm) for rustic feel
RIM_WIDTH = 0.045  # Wider rim (4.5cm)
...
def create_realistic_plate():
    ...
    profile_points = [
        ...
        (PLATE_RADIUS * 0.4, -PLATE_DEPTH),            # Bottom continues longer
        (PLATE_RADIUS * 0.65, -PLATE_DEPTH * 0.6),     # More gradual curve up
        (PLATE_RADIUS - RIM_WIDTH, -PLATE_DEPTH * 0.3), # Approaching rim
        (PLATE_RADIUS - RIM_WIDTH * 0.6, 0.0),         # Rim starts
        (PLATE_RADIUS - 0.008, 0.004),                 # More pronounced rim lip
...
def create_ceramic_material(obj):
    ...
    principled_node.inputs['Base Color'].default_value = (0.15, 0.25, 0.65, 1.0)  # Deep blue color
    
    # Ceramic properties - more matte for rustic look
    principled_node.inputs['Metallic'].default_value = 0.0
    principled_node.inputs['Roughness'].default_value = 0.4  # More matte finish
