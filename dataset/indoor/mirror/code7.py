...
frame_width = 0.07  # Increased thickness for ornate look
frame_depth = 0.04  # Increased depth
base_width = 0.6
base_depth = 0.4
base_height = 0.05
stand_width = 0.04
...
def create_brass_frame_material():
    ...
    node_bsdf.inputs['Base Color'].default_value = (0.7, 0.5, 0.2, 1.0)  # Antique brass
...
def create_mirror_material():
    """Create a reflective mirror material with slight aging"""
    ...
    node_bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
    node_bsdf.location = (0, 0)
    node_bsdf.inputs['Base Color'].default_value = (0.9, 0.9, 0.85, 1.0)  # Slightly warm tint
...
def create_aged_wood_material():
    """Create material for aged wood base"""
    mat = bpy.data.materials.new(name="AgedWood")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    nodes.clear()
    ...
    node_bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
    node_bsdf.location = (0, 0)
    node_bsdf.inputs['Base Color'].default_value = (0.15, 0.1, 0.08, 1.0)  # Very dark aged wood
...
aged_wood_mat = create_aged_wood_material()
...
base.data.materials.append(aged_wood_mat)
...
left_stand.data.materials.append(aged_wood_mat)
...
right_stand.data.materials.append(aged_wood_mat)
...
connect_left.data.materials.append(aged_wood_mat)
...
connect_right.data.materials.append(aged_wood_mat)
...
print("Vintage ornate mirror created successfully!")
