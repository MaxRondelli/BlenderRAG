...
frame_width = 0.03  # Thinner frame for modern look
frame_depth = 0.025
base_width = 0.6
base_depth = 0.4
base_height = 0.04  # Slightly thinner base
stand_width = 0.03  # Thinner stands
stand_height = 0.3
tilt_angle = 0  # degrees - slight backward tilt

# Materials
def create_black_metal_frame_material():
    """Create a black metal frame material"""
    mat = bpy.data.materials.new(name="BlackMetalFrame")
    mat.use_nodes = True
    ...
    node_bsdf.inputs['Base Color'].default_value = (0.1, 0.1, 0.12, 1.0)  # Dark charcoal
...
def create_chrome_base_material():
    """Create material for the chrome base"""
    mat = bpy.data.materials.new(name="ChromeBase")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    nodes.clear()
    ...
    node_bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
    node_bsdf.location = (0, 0)
    node_bsdf.inputs['Base Color'].default_value = (0.7, 0.7, 0.75, 1.0)  # Chrome silver
    node_bsdf.inputs['Metallic'].default_value = 1.0
...
# Create materials
black_metal_frame_mat = create_black_metal_frame_material()
mirror_mat = create_mirror_material()
chrome_base_mat = create_chrome_base_material()
...
base.data.materials.append(chrome_base_mat)
...
left_stand.data.materials.append(black_metal_frame_mat)
...
right_stand.data.materials.append(black_metal_frame_mat)
...
frame_top.data.materials.append(black_metal_frame_mat)
...
frame_bottom.data.materials.append(black_metal_frame_mat)
...
frame_left.data.materials.append(black_metal_frame_mat)
...
frame_right.data.materials.append(black_metal_frame_mat)
...
connect_left.data.materials.append(black_metal_frame_mat)
...
connect_right.data.materials.append(black_metal_frame_mat)
...
print("Modern standing mirror created successfully!")
