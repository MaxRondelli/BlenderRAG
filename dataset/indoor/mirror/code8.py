...
frame_width = 0.06  # Slightly thicker frame for ornate look
...
stand_width = 0.04
stand_height = 0.3
tilt_angle = 0  # Forward tilt instead of backward

# Materials
def create_wood_frame_material():
    """Create a mahogany wood frame material"""
    mat = bpy.data.materials.new(name="MahoganyFrame")
    mat.use_nodes = True
    ...
    node_bsdf.inputs['Base Color'].default_value = (0.2, 0.1, 0.08, 1.0)  # Rich mahogany
...
def create_brass_frame_material():
    ...
    mat = bpy.data.materials.new(name="AntiqueBrass")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    nodes.clear()
    ...
    node_bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
    node_bsdf.location = (0, 0)
    node_bsdf.inputs['Base Color'].default_value = (0.6, 0.45, 0.2, 1.0)  # Antique brass
...
def create_base_material():
    ...
    node_bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
    node_bsdf.location = (0, 0)
    node_bsdf.inputs['Base Color'].default_value = (0.15, 0.08, 0.05, 1.0)  # Very dark mahogany
...
collection = bpy.data.collections.new("Vintage_Standing_Mirror")
...
print("Vintage standing mirror created successfully!")
