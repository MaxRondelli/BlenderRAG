...
frame_width = 0.07  # Slightly thicker frame for ornate look
frame_depth = 0.04  # Deeper frame
base_width = 0.6
base_depth = 0.4
base_height = 0.05
stand_width = 0.04
...
def create_brass_frame_material():
    """Create an antique brass frame material"""
...
def create_marble_base_material():
    """Create black marble material for the base"""
    mat = bpy.data.materials.new(name="BlackMarble")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    nodes.clear()
    ...
    node_bsdf.inputs['Base Color'].default_value = (0.05, 0.05, 0.05, 1.0)  # Deep black
    node_bsdf.inputs['Roughness'].default_value = 0.1  # Very glossy
    ...
    # Color ramp for marble veining
    color_ramp = nodes.new(type='ShaderNodeValToRGB')
    color_ramp.location = (-100, 100)
    color_ramp.color_ramp.elements[0].color = (0.05, 0.05, 0.05, 1.0)  # Black
    color_ramp.color_ramp.elements[1].color = (0.3, 0.3, 0.3, 1.0)  # Dark gray veining
...
left_stand.data.materials.append(brass_frame_mat)
...
right_stand.data.materials.append(brass_frame_mat)
...
connect_left.data.materials.append(brass_frame_mat)
...
connect_right.data.materials.append(brass_frame_mat)
...
# Set up warm vintage lighting
# Main warm sunlight from side
bpy.ops.object.light_add(type='SUN', location=(3, -2, 4))
sun = bpy.context.active_object
sun.data.energy = 1.8
sun.data.color = (1.0, 0.85, 0.6)  # Warm color temperature
...
area_light.data.energy = 120
area_light.data.color = (1.0, 0.9, 0.7)  # Warm fill
area_light.data.size = 4
area_light.rotation_euler = (math.radians(80), 0, 0)

# Soft back light for brass definition
bpy.ops.object.light_add(type='AREA', location=(-1, 2, 1.5))
back_light = bpy.context.active_object
back_light.data.energy = 60
back_light.data.color = (1.0, 0.95, 0.8)  # Slightly warm
...
bg_node.inputs['Color'].default_value = (0.95, 0.9, 0.85, 1.0)  # Warm beige background
bg_node.inputs['Strength'].default_value = 0.6

print("Vintage brass mirror created successfully!")
