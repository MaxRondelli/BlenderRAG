...
wood_color = (0.15, 0.10, 0.08, 1)  # Dark espresso brown
wood_metallic = 0.15
wood_roughness = 0.25

door_color = (0.18, 0.12, 0.10, 1)  # Slightly lighter espresso for doors
door_metallic = 0.15
door_roughness = 0.2

handle_color = (0.85, 0.70, 0.35, 1)  # Gold finish
handle_metallic = 0.95
handle_roughness = 0.1

shelf_color = (0.16, 0.11, 0.09, 1)
shelf_metallic = 0.1
shelf_roughness = 0.35
...
cabinet_thickness = 0.025  # Slightly thicker panels

# COUNTERTOP - White marble style
counter_height = lower_cabinet_height
counter_thickness = 0.04
countertop_color = (0.95, 0.94, 0.92, 1)  # White marble
countertop_metallic = 0.1
...
def create_lower_cabinet(name, position, width):
    ...
    # Door handles - now on the outside, gold finish
    handle_height = 0.018
    handle_width = 0.15
    handle_depth = 0.03
...
def create_upper_cabinet(name, position, width):
    ...
    left_glass = create_box(
        ...
        (0.85, 0.88, 0.92, 0.25),  # Slightly tinted glass
        0.05, 0.02
    )
    ...
    # Door knobs (small round knobs for upper cabinets) - now on the outside, gold finish
    knob_radius = 0.014
    knob_depth = 0.025
    ...
    light_mat.node_tree.nodes["Principled BSDF"].inputs['Emission Strength'].default_value = 2.5
    cabinet_parts.append(light_strip)
    
    return cabinet_parts

# Create marble-like countertop material with veining
def create_marble_material(obj):
    ...
    # Configure noise texture for marble veining
    ...
    # Configure color ramp for marble colors
    color_ramp.color_ramp.elements[0].color = (0.92, 0.91, 0.88, 1.0)  # Light marble
    color_ramp.color_ramp.elements[1].color = (0.85, 0.83, 0.80, 1.0)  # Darker veins
    
    # Configure BSDF
    bsdf.inputs['Metallic'].default_value = countertop_metallic
    bsdf.inputs['Roughness'].default_value = countertop_roughness
...
# Apply marble material to countertop
create_marble_material(countertop)

# Countertop backsplash (now at the back) - white subway tile style
