...
# CABINET MATERIALS - Modern Black Wood Theme
wood_color = (0.08, 0.08, 0.10, 1)  # Deep black wood
wood_metallic = 0.2
wood_roughness = 0.15

door_color = (0.06, 0.06, 0.08, 1)  # Glossy black for doors
door_metallic = 0.25
door_roughness = 0.1

handle_color = (0.85, 0.65, 0.25, 1)  # Gold metallic
handle_metallic = 0.95
handle_roughness = 0.15

shelf_color = (0.10, 0.10, 0.12, 1)
shelf_metallic = 0.15
shelf_roughness = 0.2
...
cabinet_thickness = 0.03  # Slightly thicker for modern look

# COUNTERTOP
counter_height = lower_cabinet_height
counter_thickness = 0.05
countertop_color = (0.95, 0.95, 0.97, 1)  # White quartz
...
def create_lower_cabinet(name, position, width):
    ...
    # Door handles - now on the outside (longer modern handles)
    handle_height = 0.02
    handle_width = 0.18
    handle_depth = 0.03
...
def create_upper_cabinet(name, position, width):
    ...
    left_glass = create_box(
        ...
        (0.9, 0.9, 0.95, 0.2),  # Translucent glass
        0.05, 0.02
    )
    ...
    # Door knobs (gold cylindrical knobs for upper cabinets) - now on the outside
    knob_radius = 0.015
    knob_depth = 0.025
    ...
    # Under-cabinet lighting strip (warm amber) - now on the outside bottom
    ...
    # Add emission to light strip (warm amber light)
