...
# CABINET MATERIALS (Updated for lighter maple finish)
wood_color = (0.55, 0.42, 0.28, 1)  # Light maple wood
wood_metallic = 0.05
wood_roughness = 0.2

door_color = (0.58, 0.45, 0.30, 1)  # Slightly lighter maple for doors
door_metallic = 0.05
door_roughness = 0.15

handle_color = (0.9, 0.9, 0.92, 1)  # Chrome finish
handle_metallic = 0.95
handle_roughness = 0.1

shelf_color = (0.56, 0.43, 0.29, 1)
shelf_metallic = 0.05
shelf_roughness = 0.25
...
cabinet_thickness = 0.02  # Thickness of cabinet walls

# COUNTERTOP
counter_height = lower_cabinet_height
counter_thickness = 0.04
countertop_color = (0.25, 0.25, 0.28, 1)  # Dark granite-like
countertop_metallic = 0.5
countertop_roughness = 0.15
...
def create_lower_cabinet(name, position, width):
    ...
    # Doors (2 doors) - now facing outward with thicker doors
    door_width = (width - 0.01) / 2  # Small gap between doors
    door_height = lower_cabinet_height - 0.04
    door_y = y - lower_cabinet_depth/2 + 0.015  # Slightly thicker door position
    door_thick = 0.03  # Increased door thickness
    
    # Left door
    left_door = create_box(
        f"{name}_Door_Left",
        (x - width/4, door_y, z + lower_cabinet_height/2),
        (door_width, door_thick, door_height),
    ...
    # Door handles - now on the outside with chrome finish
    handle_height = 0.018  # Slightly taller handles
    handle_width = 0.14  # Slightly wider handles
    handle_depth = 0.03
    handle_y = door_y - door_thick/2 - handle_depth/2
...
def create_upper_cabinet(name, position, width):
    ...
    # Glass doors (2 doors) - now facing outward with thicker doors
    door_width = (width - 0.01) / 2
    door_height = upper_cabinet_height - 0.04
    door_y = y - upper_cabinet_depth/2 + 0.015  # Thicker door position
    door_thick = 0.03  # Increased door thickness
    ...
    # Door knobs (small round knobs for upper cabinets) - now on the outside with chrome
    knob_radius = 0.014  # Slightly larger knobs
    knob_depth = 0.025
    knob_y = door_y - door_thick/2 - knob_depth/2
