...
# CABINET MATERIALS - Darker espresso wood
wood_color = (0.18, 0.12, 0.08, 1)  # Dark espresso brown
wood_metallic = 0.1
wood_roughness = 0.25

door_color = (0.22, 0.15, 0.10, 1)  # Slightly lighter espresso for doors
door_metallic = 0.1
door_roughness = 0.2

# Brass hardware instead of steel
handle_color = (0.8, 0.65, 0.3, 1)  # Warm brass
handle_metallic = 0.9
handle_roughness = 0.2

# Glass shelves instead of wood
shelf_color = (0.9, 0.95, 0.98, 0.15)  # Clear glass with slight transparency
shelf_metallic = 0.05
shelf_roughness = 0.05
...
cabinet_thickness = 0.025  # Slightly thicker walls for premium look
...
def create_lower_cabinet(name, position, width):
    ...
    # Internal shelf - using wood color for lower cabinets
    shelf = create_box(
        f"{name}_Shelf",
    ...
    # Door handles - now brass colored and on the outside
    handle_height = 0.015
    handle_width = 0.12
    handle_depth = 0.025
...
def create_upper_cabinet(name, position, width):
    ...
    # Internal glass shelves (2 shelves for upper cabinets)
    shelf_1 = create_box(
        f"{name}_Shelf_1",
        (x, y, z + upper_cabinet_height * 0.33),
        (width - 2*cabinet_thickness - 0.02, upper_cabinet_depth - cabinet_thickness - 0.02, 0.01),
    ...
    # Door knobs (brass knobs for upper cabinets) - now on the outside
    knob_radius = 0.012
    knob_depth = 0.02
