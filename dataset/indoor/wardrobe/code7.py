...
# Handle dimensions - increased depth for better ergonomics
HANDLE_WIDTH = 0.10  # 10cm
HANDLE_HEIGHT = 0.015  # 1.5cm
HANDLE_DEPTH = 0.035  # 3.5cm protrusion (was 2.5cm)

# Door handle dimensions (vertical) - increased depth
DOOR_HANDLE_WIDTH = 0.015  # 1.5cm wide
DOOR_HANDLE_HEIGHT = 0.15  # 15cm tall
DOOR_HANDLE_DEPTH = 0.035  # 3.5cm protrusion (was 2.5cm)
...
def create_doors(divider_z):
    ...
    door_thickness = 0.025  # Slightly thicker doors
...
def create_drawers():
    ...
    drawer_depth = 0.025  # Slightly thicker drawer fronts
...
def create_wood_material(name, base_color):
    ...
    noise_texture.inputs['Scale'].default_value = 20.0  # Finer grain
    ...
    lighter_color = (base_color[0] * 1.15, base_color[1] * 1.1, base_color[2] * 1.05, 1.0)
    color_ramp.color_ramp.elements[1].color = lighter_color
    
    principled_node.inputs['Metallic'].default_value = 0.0
    principled_node.inputs['Roughness'].default_value = 0.25  # Slightly more polished
...
def create_metal_material(name):
    ...
    principled_node.inputs['Base Color'].default_value = (0.6, 0.6, 0.65, 1.0)  # Steel color
...
def apply_materials(frame_parts, doors, door_handles, drawers, drawer_handles):
    ...
    # Dark ebony wood color
    ebony_color = (0.15, 0.12, 0.10, 1.0)  # Much darker than original walnut
    wood_mat = create_wood_material("EbonyWood", ebony_color)
    
    # Brushed steel for handles
    metal_mat = create_metal_material("BrushedSteel")
    
    # Apply wood material to main components
...
if __name__ == "__main__":
    print("Creating modern wardrobe variation...")
    ...
    print("\nModern wardrobe variation created successfully!")
    ...
    print("Features: Ebony wood finish, brushed steel handles, thicker panels")