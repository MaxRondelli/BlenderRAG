...
# Panel thickness - INCREASED for luxury feel
...
# Handle dimensions - ENHANCED for luxury
HANDLE_WIDTH = 0.12  # 12cm (was 10cm)
HANDLE_HEIGHT = 0.020  # 2cm (was 1.5cm)
HANDLE_DEPTH = 0.030  # 3cm protrusion (was 2.5cm)

# Door handle dimensions (vertical) - ENHANCED
DOOR_HANDLE_WIDTH = 0.020  # 2cm wide (was 1.5cm)
...
def create_doors(divider_z):
    ...
    for i in range(door_count):
        ...
        bevel_mod.width = 0.003  # Slightly larger bevel
...
def create_door_handles(doors):
    ...
    for i, door in enumerate(doors):
        ...
        bevel_mod.width = 0.008  # More rounded for luxury feel
...
def create_drawers():
    ...
    drawer_depth = 0.025  # 2.5cm thick drawer fronts (slightly thicker)
...
def create_drawer_handles(drawers):
    ...
    for i, drawer in enumerate(drawers):
        ...
        # Add bevel for rounded edges
        bevel_mod = handle.modifiers.new(name="Bevel", type='BEVEL')
        bevel_mod.width = 0.006  # More rounded for luxury feel
        bevel_mod.segments = 6
        bpy.ops.object.shade_smooth()
...
def create_mahogany_material(name, base_color):
    """Creates a mahogany wood material with enhanced grain texture"""
    ...
    # Primary wood grain texture
    noise_texture = nodes.new(type='ShaderNodeTexNoise')
    noise_texture.location = (-600, 0)
    noise_texture.inputs['Scale'].default_value = 25.0  # Finer grain
    noise_texture.inputs['Detail'].default_value = 15.0  # More detail
    noise_texture.inputs['Roughness'].default_value = 0.4
    
    # Secondary texture for wood rings
    wave_texture = nodes.new(type='ShaderNodeTexWave')
    wave_texture.location = (-600, -300)
    wave_texture.inputs['Scale'].default_value = 8.0
    wave_texture.inputs['Distortion'].default_value = 2.0
    wave_texture.wave_profile = 'TRI'
    
    # Mix the textures
    mix_node = nodes.new(type='ShaderNodeMix')
    mix_node.location = (-300, 0)
    mix_node.data_type = 'RGBA'
    mix_node.inputs[0].default_value = 0.3  # Factor
    
    color_ramp = nodes.new(type='ShaderNodeValToRGB')
    color_ramp.location = (-100, 0)
    color_ramp.color_ramp.elements[0].color = base_color
    lighter_color = (base_color[0] * 1.3, base_color[1] * 1.2, base_color[2] * 1.1, 1.0)
    color_ramp.color_ramp.elements[1].color = lighter_color
    
    principled_node.inputs['Metallic'].default_value = 0.0
    principled_node.inputs['Roughness'].default_value = 0.2  # Shinier finish
    
    # Connect nodes
    links.new(noise_texture.outputs['Fac'], mix_node.inputs[6])
    links.new(wave_texture.outputs['Fac'], mix_node.inputs[7])
    links.new(mix_node.outputs['Result'], color_ramp.inputs['Fac'])
...
def create_brass_material():
    """Creates a brass material for handles"""
    ...
    # Brass color - golden
    principled_node.inputs['Base Color'].default_value = (0.85, 0.65, 0.25, 1.0)
...
def apply_materials(frame_and_doors, handles):
    """Apply mahogany wood material to frame/doors and brass to handles"""
    mahogany_color = (0.45, 0.18, 0.12, 1.0)  # Rich reddish-brown mahogany
    wood_mat = create_mahogany_material("MahoganyWood", mahogany_color)
    brass_mat = create_brass_material()
    
    # Apply wood material to frame and doors
    for obj in frame_and_doors:
        if obj.data.materials:
            obj.data.materials[0] = wood_mat
        else:
            obj.data.materials.append(wood_mat)
    
    # Apply brass material to handles
    for obj in handles:
        if obj.data.materials:
            obj.data.materials[0] = brass_mat
        else:
...
if __name__ == "__main__":
    print("Creating luxury mahogany wardrobe...")
    ...
    # Separate wood components from handles
    wood_components = frame_parts + doors + drawers
    handle_components = door_handles + drawer_handles
    
    # Apply materials
    apply_materials(wood_components, handle_components)
    
    # Setup scene
    setup_scene()
    
    print("\nLuxury mahogany wardrobe created successfully!")
    print(f"Dimensions: {WARDROBE_WIDTH*100:.0f}cm W x {WARDROBE_DEPTH*100:.0f}cm D x {WARDROBE_HEIGHT*100:.0f}cm H")
    print(f"3 doors in upper section with brass handles")
    print(f"{DRAWER_COUNT} rows x 3 columns = {DRAWER_COUNT * 3} drawers total with brass handles")
    print("Enhanced with thicker panels and luxury finishes!")