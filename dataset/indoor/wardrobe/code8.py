...
# Wardrobe dimensions (in meters) - modern sleeker proportions
WARDROBE_WIDTH = 1.50  # 150cm width (3 sections)
WARDROBE_DEPTH = 0.55  # 55cm depth - slightly shallower for modern look
WARDROBE_HEIGHT = 2.00  # 200cm height

# Panel thickness - thinner for modern aesthetic
PANEL_THICKNESS = 0.018  # 1.8cm - thinner panels

# Door section (upper part with doors)
DOOR_HEIGHT = 1.40  # 140cm for door section
DOOR_GAP = 0.004  # 4mm gap between doors - slightly wider

# Drawer section (lower part)
DRAWER_SECTION_HEIGHT = WARDROBE_HEIGHT - DOOR_HEIGHT - PANEL_THICKNESS
DRAWER_COUNT = 3  # 3 rows of drawers
DRAWER_ROWS = 2  # 2 drawers per row
DRAWER_GAP = 0.004  # 4mm gap
DRAWER_HEIGHT = (DRAWER_SECTION_HEIGHT - PANEL_THICKNESS * 2 - DRAWER_GAP * (DRAWER_COUNT - 1)) / DRAWER_COUNT

# Handle dimensions - more refined
HANDLE_WIDTH = 0.12  # 12cm - slightly longer
HANDLE_HEIGHT = 0.012  # 1.2cm - thinner
...
def create_doors(divider_z):
    ...
    door_thickness = 0.018  # Thinner doors for modern look
    ...
    for i in range(door_count):
        ...
        # Add bevel for rounded edges - more subtle
...
def create_drawers():
    ...
    drawer_depth = 0.018  # Thinner drawer fronts
...
def create_wood_material(name, base_color):
    """Creates a modern ebony wood material with fine grain"""
    ...
    # Fine wood grain texture for ebony
    ...
    # Subtle variation for ebony
    lighter_color = (base_color[0] * 1.1, base_color[1] * 1.08, base_color[2] * 1.05, 1.0)
    color_ramp.color_ramp.elements[1].color = lighter_color
    
    # Modern finish properties
    principled_node.inputs['Metallic'].default_value = 0.0
    principled_node.inputs['Roughness'].default_value = 0.2  # More polished
...
def apply_materials(components):
    """Apply dark ebony wood material to all components"""
    ebony_color = (0.15, 0.12, 0.10, 1.0)  # Very dark brown for ebony
