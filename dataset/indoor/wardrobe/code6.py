...
HANDLE_HEIGHT = 0.012  # 1.2cm (reduced height)
HANDLE_DEPTH = 0.020  # 2cm protrusion (reduced)

# Door handle dimensions (vertical) - sleeker
DOOR_HANDLE_WIDTH = 0.012  # 1.2cm wide (reduced)
DOOR_HANDLE_HEIGHT = 0.18  # 18cm tall (increased)
DOOR_HANDLE_DEPTH = 0.020  # 2cm protrusion (reduced)
...
def create_doors(divider_z):
    ...
    door_thickness = 0.025  # 2.5cm thick doors (increased)
    ...
    for i in range(door_count):
        ...
        bevel_mod.width = 0.003  # Increased bevel
...
def create_drawers():
    ...
    drawer_depth = 0.025  # 2.5cm thick drawer fronts (increased)
...
def create_modern_material(name, base_color):
    """Creates a modern matte material"""
    ...
    # Set modern black finish properties
    principled_node.inputs['Base Color'].default_value = base_color
    principled_node.inputs['Metallic'].default_value = 0.1  # Slightly metallic
    principled_node.inputs['Roughness'].default_value = 0.6  # Matte finish
...
def create_handle_material():
    ...
    noise_texture.inputs['Scale'].default_value = 200.0
    noise_texture.inputs['Detail'].default_value = 16.0
    noise_texture.inputs['Roughness'].default_value = 0.4
    
    color_ramp = nodes.new(type='ShaderNodeValToRGB')
    color_ramp.location = (0, 0)
    color_ramp.color_ramp.elements[0].color = (0.7, 0.7, 0.7, 1.0)  # Dark metal
    color_ramp.color_ramp.elements[1].color = (0.9, 0.9, 0.9, 1.0)  # Light metal
...
def apply_materials(components):
    """Apply modern black material to all components"""
    black_color = (0.08, 0.08, 0.08, 1.0)  # Modern black
    modern_mat = create_modern_material("ModernBlack", black_color)
    metal_mat = create_handle_material()
    ...
    for obj in components:
        ...
        if "Handle" in obj.name:
            ...
            # Apply black material to everything else
            if obj.data.materials:
                obj.data.materials[0] = modern_mat
            else:
                obj.data.materials.append(modern_mat)
...
def setup_scene():
    ...
    # Key light
    bpy.ops.object.light_add(type='AREA', location=(3.5, -3, 4))
    key_light = bpy.context.active_object
    key_light.data.energy = 400
    key_light.data.size = 3.5
    
    # Fill light
    bpy.ops.object.light_add(type='AREA', location=(-2, -2.5, 3.5))
...
if __name__ == "__main__":
    print("Creating modern wardrobe...")
