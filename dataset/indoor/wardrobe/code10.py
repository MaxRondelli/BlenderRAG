...
# Panel thickness - increased for more robust look
...
# Handle dimensions - sleeker metal handles
HANDLE_WIDTH = 0.12  # 12cm (slightly longer)
HANDLE_HEIGHT = 0.008  # 0.8cm (thinner profile)
HANDLE_DEPTH = 0.020  # 2cm protrusion

# Door handle dimensions (vertical) - sleeker design
DOOR_HANDLE_WIDTH = 0.010  # 1cm wide (thinner)
DOOR_HANDLE_HEIGHT = 0.18  # 18cm tall (longer)
...
def create_door_handles(doors):
    """Creates vertical metal handles for the doors"""
    handles = []
    ...
    for i, door in enumerate(doors):
        ...
        # Add bevel for rounded edges - more pronounced for metal look
...
def create_drawers():
    ...
    drawer_depth = 0.025  # 2.5cm thick drawer fronts (thicker)
...
def create_drawer_handles(drawers):
    """Creates sleek metal handles for all drawers"""
    handles = []
    ...
    for i, drawer in enumerate(drawers):
        ...
        # Add bevel for rounded edges - sleek metal finish
        bevel_mod = handle.modifiers.new(name="Bevel", type='BEVEL')
        bevel_mod.width = 0.002
        bevel_mod.segments = 6
        bpy.ops.object.shade_smooth()
...
def create_ebony_wood_material():
    """Creates a dark ebony wood material"""
    mat = bpy.data.materials.new(name="EbonyWood")
    mat.use_nodes = True
    ...
    # Very dark ebony colors
    color_ramp.color_ramp.elements[0].color = (0.08, 0.05, 0.03, 1.0)  # Very dark brown
    color_ramp.color_ramp.elements[1].color = (0.15, 0.10, 0.06, 1.0)  # Slightly lighter
    
    principled_node.inputs['Metallic'].default_value = 0.0
    principled_node.inputs['Roughness'].default_value = 0.2  # More polished finish
...
def create_black_metal_material():
    """Creates a black brushed metal material for handles"""
    ...
    principled_node.inputs['Base Color'].default_value = (0.05, 0.05, 0.05, 1.0)  # Very dark gray/black
    principled_node.inputs['Metallic'].default_value = 0.9
    principled_node.inputs['Roughness'].default_value = 0.3
    
    links.new(noise_texture.outputs['Fac'], principled_node.inputs['Roughness'])
    links.new(principled_node.outputs['BSDF'], output_node.inputs['Surface'])
    
    return mat

def apply_materials(frame_parts, doors, door_handles, drawers, drawer_handles):
    """Apply materials to different components"""
    ebony_mat = create_ebony_wood_material()
    metal_mat = create_black_metal_material()
    
    # Apply ebony wood to frame, doors, and drawers
    wood_components = frame_parts + doors + drawers
    for obj in wood_components:
        if obj.data.materials:
            obj.data.materials[0] = ebony_mat
        else:
            obj.data.materials.append(ebony_mat)
    
    # Apply black metal to all handles
    metal_components = door_handles + drawer_handles
    for obj in metal_components:
        if obj.data.materials:
...
if __name__ == "__main__":
    print("Creating modern ebony wardrobe...")
    ...
    # Apply materials
    apply_materials(frame_parts, doors, door_handles, drawers, drawer_handles)
    
    # Setup scene
    setup_scene()
    
    print("\nModern ebony wardrobe created successfully!")
    print(f"Dimensions: {WARDROBE_WIDTH*100:.0f}cm W x {WARDROBE_DEPTH*100:.0f}cm D x {WARDROBE_HEIGHT*100:.0f}cm H")
    print(f"3 doors in upper section with black metal handles")
    print(f"{DRAWER_COUNT} rows x 3 columns = {DRAWER_COUNT * 3} drawers total")
    print("Dark ebony finish with sleek black metal hardware!")