...
# Panel thickness - Made thinner for modern look
PANEL_THICKNESS = 0.018  # 1.8cm (reduced from 2.5cm)
...
# Handle dimensions - Made more modern/metallic
HANDLE_WIDTH = 0.12  # 12cm (increased)
HANDLE_HEIGHT = 0.008  # 0.8cm (made thinner/sleeker)
HANDLE_DEPTH = 0.015  # 1.5cm protrusion (reduced)

# Door handle dimensions (vertical) - Made more modern
DOOR_HANDLE_WIDTH = 0.008  # 0.8cm wide (thinner)
DOOR_HANDLE_HEIGHT = 0.18  # 18cm tall (increased)
DOOR_HANDLE_DEPTH = 0.015  # 1.5cm protrusion (reduced)
...
def create_doors(divider_z):
    ...
    door_thickness = 0.018  # 1.8cm thick doors (thinner)
...
def create_drawers():
    ...
    drawer_depth = 0.018  # 1.8cm thick drawer fronts (thinner)
...
def create_black_modern_material():
    """Creates a modern black material with subtle texture"""
    mat = bpy.data.materials.new(name="ModernBlack")
    mat.use_nodes = True
    ...
    color_ramp.color_ramp.elements[0].color = (0.08, 0.08, 0.08, 1.0)  # Dark black
    color_ramp.color_ramp.elements[1].color = (0.15, 0.15, 0.15, 1.0)  # Slightly lighter black
...
def create_metallic_material():
    """Creates a brushed metal material for handles"""
    mat = bpy.data.materials.new(name="BrushedMetal")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
    ...
    wave_texture.inputs['Scale'].default_value = 100.0
    wave_texture.wave_profile = 'SAW'  # Changed from 'BANDS' to 'SAW'
    
    color_ramp = nodes.new(type='ShaderNodeValToRGB')
    color_ramp.location = (-100, 0)
    color_ramp.color_ramp.elements[0].color = (0.7, 0.7, 0.75, 1.0)  # Steel color
    color_ramp.color_ramp.elements[1].color = (0.85, 0.85, 0.9, 1.0)  # Lighter steel
    
    principled_node.inputs['Metallic'].default_value = 0.9
    principled_node.inputs['Roughness'].default_value = 0.2
    principled_node.inputs['IOR'].default_value = 2.5
...
def apply_materials(frame_parts, doors, door_handles, drawers, drawer_handles):
    ...
    black_mat = create_black_modern_material()
    metallic_mat = create_metallic_material()
    
    # Apply black material to main structure and doors/drawers
    ...
    # Apply metallic material to handles
...
if __name__ == "__main__":
    print("Creating modern black wardrobe...")
    ...
    print("\nModern black wardrobe created successfully!")
    ...
    print("Modern black finish with brushed metal handles!")