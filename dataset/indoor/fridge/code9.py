...
def create_fridge_body(location=(0, 0, 0), size=(1.1, 0.8, 1.8)):
    ...
    # Create material for body (black stainless steel)
    body_mat = create_material("FridgeMaterial", (0.15, 0.15, 0.18), metallic=0.8, roughness=0.2)
...
def create_fridge_door(location, size, name="Door"):
    ...
    door_mat = create_material(f"{name}_Material", (0.12, 0.12, 0.15), metallic=0.85, roughness=0.15)
...
def create_door_handle(location, size, orientation='vertical'):
    ...
    # Create brushed metal material for handle
    handle_mat = create_material("HandleMaterial", (0.3, 0.3, 0.32), metallic=0.95, roughness=0.15)
...
def create_dispenser_panel(location):
    ...
    dispenser_mat = create_material("DispenserMaterial", (0.08, 0.08, 0.1), metallic=0.0, roughness=0.7)
    ...
    opening_mat = create_material("OpeningMaterial", (0.03, 0.03, 0.03), metallic=0.0, roughness=0.9)
