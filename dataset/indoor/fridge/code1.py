...
def create_fridge_body(location=(0, 0, 0), size=(0.9, 0.8, 1.8)):
    ...
    body_mat = create_material("FridgeMaterial", (0.05, 0.05, 0.05), metallic=0.1, roughness=0.2)
...
def create_fridge_door(location, size, name="Door"):
    ...
    # Create material for door (dark gray)
    door_mat = create_material(f"{name}_Material", (0.08, 0.08, 0.08), metallic=0.3, roughness=0.15)
...
def create_door_handle(location, size, orientation='vertical'):
    ...
    handle_mat = create_material("HandleMaterial", (0.8, 0.8, 0.85), metallic=0.95, roughness=0.3)
...
def create_dispenser_panel(location):
    ...
    dispenser_mat = create_material("DispenserMaterial", (0.02, 0.02, 0.02), metallic=0.0, roughness=0.7)
    ...
    opening_mat = create_material("OpeningMaterial", (0.01, 0.01, 0.01), metallic=0.0, roughness=0.9)
