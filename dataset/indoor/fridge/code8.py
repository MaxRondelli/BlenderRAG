...
def create_fridge_body(location=(0, 0, 0), size=(1.1, 0.8, 1.8)):
    ...
    # Create material for body (black/dark metallic)
...
def create_fridge_door(location, size, name="Door"):
    ...
    # Create material for door
    door_mat = create_material(f"{name}_Material", (0.1, 0.1, 0.12), metallic=0.9, roughness=0.15)
...
def create_door_handle(location, size, orientation='vertical'):
    ...
    # Create metallic material for handle
...
def create_dispenser_panel(location):
    ...
    bpy.ops.mesh.primitive_cube_add(
        location=location,
        scale=(0.12, 0.015, 0.18)
    )
    dispenser = bpy.context.active_object
    dispenser.name = "Dispenser"
    
    # Dark plastic material
    dispenser_mat = create_material("DispenserMaterial", (0.05, 0.05, 0.08), metallic=0.1, roughness=0.7)
    dispenser.data.materials.append(dispenser_mat)
    
    # Dispenser opening
    bpy.ops.mesh.primitive_cube_add(
        location=(location[0], location[1] + 0.01, location[2] - 0.08),
        scale=(0.06, 0.012, 0.06)
    )
