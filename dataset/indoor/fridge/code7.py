...
def create_fridge_body(location=(0, 0, 0), size=(0.9, 0.8, 1.8)):
    ...
    body_mat = create_material("FridgeMaterial", (0.08, 0.08, 0.1), metallic=0.1, roughness=0.7)
...
def create_fridge_door(location, size, name="Door"):
    ...
    door_mat = create_material(f"{name}_Material", (0.1, 0.1, 0.12), metallic=0.15, roughness=0.6)
...
def create_door_handle(location, size, orientation='vertical'):
    ...
    handle_mat = create_material("HandleMaterial", (0.72, 0.45, 0.2), metallic=0.8, roughness=0.3)
...
def create_fridge(location=(0, 0, 0)):
    ...
    # Dimensions (taller than original)
    fridge_width = 0.9
    fridge_depth = 0.8
    fridge_height = 2.0
...
def create_dispenser_panel(location):
    ...
    # Dark material with copper accent
    dispenser_mat = create_material("DispenserMaterial", (0.05, 0.05, 0.08), metallic=0.2, roughness=0.5)
