...
def create_fridge_body(location=(0, 0, 0), size=(0.9, 0.8, 1.8)):
    ...
    # Create material for body (retro cream color)
    body_mat = create_material("FridgeMaterial", (0.95, 0.92, 0.85), metallic=0.1, roughness=0.6)
...
def create_fridge_door(location, size, name="Door"):
    ...
    # Create material for door (slightly darker cream)
    door_mat = create_material(f"{name}_Material", (0.92, 0.88, 0.82), metallic=0.15, roughness=0.5)
...
def create_door_handle(location, size, orientation='vertical'):
    ...
    if orientation == 'vertical':
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            scale=(0.12, size/2, 0.025)
        )
    else:
        bpy.ops.mesh.primitive_cylinder_add(
            location=location,
            rotation=(0, 0, math.radians(90)),
            scale=(0.025, size/2, 0.025)
        )
    
    handle = bpy.context.active_object
    handle.name = "Handle"
    
    # Create chrome material for handle
    handle_mat = create_material("HandleMaterial", (0.8, 0.82, 0.85), metallic=0.95, roughness=0.1)
...
def create_dispenser_panel(location):
    ...
    # Chrome material for retro look
    dispenser_mat = create_material("DispenserMaterial", (0.75, 0.78, 0.82), metallic=0.9, roughness=0.2)
    ...
    opening_mat = create_material("OpeningMaterial", (0.05, 0.05, 0.05), metallic=0.0, roughness=0.8)
...
def add_door_details(freezer_door, main_door):
    ...
    # Add bevel modifier for more rounded edges
