...
def create_fridge_body(location=(0, 0, 0), size=(0.9, 0.8, 2.0)):
    ...
    # Create material for body (matte black)
    body_mat = create_material("FridgeMaterial", (0.1, 0.1, 0.12), metallic=0.1, roughness=0.7)
...
def create_fridge_door(location, size, name="Door"):
    ...
    # Create material for door (slightly lighter black)
    door_mat = create_material(f"{name}_Material", (0.15, 0.15, 0.18), metallic=0.2, roughness=0.6)
...
def create_door_handle(location, size, orientation='vertical'):
    ...
    if orientation == 'vertical':
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            scale=(0.1, size/2, 0.02)
        )
    else:
        bpy.ops.mesh.primitive_cylinder_add(
            location=location,
            rotation=(0, 0, math.radians(90)),
            scale=(0.02, size/2, 0.02)
        )
    
    handle = bpy.context.active_object
    handle.name = "Handle"
    
    # Create bright metallic material for handle (silver)
    handle_mat = create_material("HandleMaterial", (0.8, 0.8, 0.82), metallic=0.95, roughness=0.1)
...
def create_dispenser_panel(location):
    ...
    dispenser_mat = create_material("DispenserMaterial", (0.05, 0.05, 0.07), metallic=0.0, roughness=0.8)
    ...
    opening_mat = create_material("OpeningMaterial", (0.02, 0.02, 0.02), metallic=0.0, roughness=0.9)
