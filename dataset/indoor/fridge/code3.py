...
def create_fridge_body(location=(0, 0, 0), size=(0.9, 0.8, 1.8)):
    ...
    body_mat = create_material("FridgeMaterial", (0.2, 0.2, 0.22), metallic=0.85, roughness=0.15)
...
def create_fridge_door(location, size, name="Door"):
    ...
    door_mat = create_material(f"{name}_Material", (0.15, 0.15, 0.18), metallic=0.9, roughness=0.1)
...
def create_door_handle(location, size, orientation='vertical'):
    ...
    # Create dark metallic material for handle
    handle_mat = create_material("HandleMaterial", (0.3, 0.3, 0.32), metallic=0.95, roughness=0.1)
